...
- **An unattended `[[ASK]]` had no deadline — the one leftover that could
  ...
  the ONE writer, called from the THREE doors through which Josh proves he is
  ...
  every other state key. The CLI sets nothing and is unchanged, and that is a
  stated BOUNDARY rather than a claim about the world: `relay.py` driven from
  Task Scheduler really is unattended and would wedge, and there is no flag
  for it today. Four things fell out of it:
...
- **One loop now** (was: two duplicated loops, extracted 2026-08-16):
  ...
  tests — see `tests/` (2593 tests across 75 suites, all runnable as plain scripts, or `python tests/run_all.py` for the lot).
...
**Token-free first**: `tests/` holds 2593 tests across 75 suites (plus three
