...
- **An `assertIn` on a source segment sees the name and never the meaning.**
  ...
  three were green across the whole 2597-test suite. `assertEqual` on the
...
- **One loop now** (was: two duplicated loops, extracted 2026-08-16):
  ...
  tests — see `tests/` (2597 tests across 75 suites, all runnable as plain scripts, or `python tests/run_all.py` for the lot).
...
**Token-free first**: `tests/` holds 2597 tests across 75 suites (plus three
