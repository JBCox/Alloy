...
| `memory.py` | **Notes that outlive one conversation** (Wave 3, 2026-08-27). Standalone like export/fork/stats -- stdlib only, imports nothing from relay/app, never raises out of a public function, and it keeps NO module-level root: relay owns `MEMORY_DIR` and passes it in, because fork.py's gotcha is that a second default is how two halves of the app disagree about where the data lives. `BASE_DIR/memory/` is a **sibling** of `sessions/` (a child would ship a phantom rail row the two-step delete would `rmtree` -- there is a test that builds one and watches it appear). Markdown is the ONLY source of truth: no persisted index, because at a few hundred short entries the rollup costs under a millisecond to recompute and an index would buy nothing while adding a whole staleness class. `project_key` carries a sha1 of the **normcased** path AND normcases the basename (normcasing only the digest gives two spellings of one folder one hash and two filenames -- the exact collision the sha1 was added to prevent, arriving through the other half of the key). **A chat reads and writes exactly ONE scope** -- its project, or `global` in a scratch chat -- and the single crossing is by KIND, not scope: a note **Josh** wrote globally reaches every chat, a note a SEAT wrote globally does not, which is the cross-project poisoning path closed by construction. Every write is read-modify-write on a file several chats share, so it takes a real cross-process lock (stale locks are BROKEN, not waited out, or a killed process disables memory forever) -- last-rename-wins would silently lose a note, and Alloy runs several chats at once by design. Eviction never touches Josh's own notes and says out loud when it dropped a seat's; truncation announces itself in all three places it can happen (an oversized file, a trimmed search, a trimmed preamble block), and a file too large to read in full REFUSES to be rewritten rather than deleting everything past the cut. Four ways in: `/remember`/`/forget`/`/memory` (relay), the `#memModal` (app), `archive_objective`'s settled-objective record (zero CLI calls), and the seats' own `[[REMEMBER]]`/`[[RECALL]]`. |
| `schedule.py` | **Scheduled rooms** (Wave 4, 2026-08-27 — the LAST wave). Standalone like export/fork/stats/memory: stdlib only, imports nothing from relay or app, no reader raises, and it owns **no root directory** — relay's `SESSIONS_DIR` is the one answer to where Alloy's data lives, so callers pass the path in and `app._schedule_path()` JOINS it at call time (the `write_tabs` lesson: a module constant captured at import survives a test's redirect and writes into Josh's real `sessions/`). Four kinds rather than cron — `once` / `interval` / `daily` / `weekly` — each a handful of validated fields instead of a mini-language whose parser is its own bug surface. Four rules carry it. **A missed window is SKIPPED and said, never fired late**: Alloy is closed most nights, a 01:00 job found at 09:15 has one honest answer that is not "start now", and both branches recompute the next window from NOW, so three days of downtime produce ZERO runs rather than three (`MISSED_GRACE_S` is the width of "near enough" — a poll that slipped, a laptop that slept). **Every time is naive LOCAL and daily/weekly RE-ANCHOR** to the configured HH:MM via `_at_time` rather than accumulating, so a stored `next_run` that is wrong for any reason corrects itself; what naive local does NOT fix is the two hours a year that are ambiguous or do not exist, which the grace window covers and the docstring says out loud rather than claiming DST-correctness it does not have. **The acknowledgement is re-checked against the room's CURRENT grants at FIRE time** — rooms are saved by NAME and overwriting one is documented behaviour, so an ack from March cannot speak for a room that gained Full access in August (`ack_gap`; narrowing is silent, widening stops the fire and says which grant). **Policy here, normalization in relay**: `grants_for` takes ALREADY-normalized axes and `app.Api._room_axes` does the normalizing with relay's own functions, because a second copy of `normalize_desktop` next to the acknowledgement would be `browser_mcp._confine`'s drift on the control that decides what runs unattended. `GRANTS` is the standing-grant set — Full permission, connected apps, desktop `full` **and `allowlist`** (the rung that clicks without asking), browser `full`, and an unbounded Keep Improving recipe. `unattended_notes` states the opposite: controls that do NOTHING at 01:00 — every Ask rung denies, a `permission` check-in makes the run WAIT, an unanswered `[[ASK]]` is not answered by anybody and the run carries on without it after `relay.ask_wait_limit` seconds (`unattended_notes` takes that number as an ARGUMENT, computed by relay for THIS room — a Keep Improving room's cap is its check-in interval, so a flat `ASK_WAIT_MAX` in the modal would promise ten minutes where the engine allows five), and at `auto`/`full` every seat already has a shell, so the desktop and browser lists bound what Alloy drives and not what a seat can do with a command line. `claim` advances the record BEFORE the run starts (`run_checkin`'s rule) and is a compare-and-set; `save_schedule` carries `runs`/`misses`/`last_run`/`last_result`/`created` over from the row it replaces, because the caller sends a FORM and an edit that wiped the history reported a nightly job as having never run. |
| `report.py` | **What happened while you were away** (2026-09-05) — the card a chat opens with when it ran without Josh. Standalone like export/fork/stats/memory/schedule: stdlib only, imports nothing from relay or app, owns no root, and no public function raises. It reads ONE already-loaded `meta.json` dict and nothing else — no file, no subprocess, no git — which is what makes `Api.session_report` a SYNCHRONOUS bridge method and the whole card cost one meta read. The three git facts a reader might expect here are the Changes tab's job; a second, less careful git reader competing with it is how two surfaces come to disagree about one folder. **Policy here, normalization in relay**: `build(meta, facts)` takes `supervisor_status`, `was_interrupted`, `running` and `SUPERVISOR_TRACE_MAX` already computed by the bridge, because `supervisor_status`' precedence is documented as the one a re-deriving reader gets wrong. Six rules carry it, and five came off measurements of the real `sessions/` folder rather than the feature request. **A count taken from the trace is a FLOOR and says so** — the log keeps 120 entries and trims the OLDEST, and a two-hour run is already at 101, so it under-reports exactly as the night gets longer; the caveat rides each figure (`≥5`) rather than sitting in a note above the chips. **Waves are counted from `plan_created`, never from a wave NUMBER** (see the gotcha). **Only ONE thing here may be called a duration**: Keep Improving's accumulated `elapsed_s`. Everything else is a SPAN between two instants — `created`→`updated` measures the CHAT (one real session: 72,949s for a run of minutes) and the control log's first→last bracket includes every idle gap (measured: 39,307s across ELEVEN entries) AND shrinks as the log is trimmed. **A number nobody reported is None, never 0** — `usage` is absent on 34 of 52 real sessions, Gemini and OpenCode report no cost ever, and `record_usage` SEEDS `total_cost_usd: 0.0`, so the total's presence proves nothing and only a real per-seat or per-kind cost counts as evidence. **Tokens are deliberately absent and the card says so** — `stats.py` is the one reader that handles `MIN_TRUSTED_BASIS` and the two cache conventions, and a second wrong copy would be worse than the gap. `available` (is there anything to report) and `show` (does the chat OPEN with the card) are separate questions: the button follows the first, the card the second, and pressing the button when they disagree paints the refusal SENTENCE rather than doing nothing. `KINDS` is a deliberate second copy of `relay.STARTED_BY_KINDS`, kept honest by a parity test rather than by its comment. |
...
Memory: every seat may END a reply with `[[REMEMBER: what to keep]]` or
`[[RECALL: words]]`. Both are scoped to the chat's OWN memory scope (its
project, or `global` in a scratch chat) and reach nothing else; recall results
go to the REQUESTER only, like a spawned helper's. One of each per reply, and
both are in `KNOWN_DIRECTIVES` (unlike TASK/DONE/OBJECTIVE, which are opted in
by their own parsers) because they are ordinary seat capabilities, not
orchestration authority. Wired at all FOUR `handle_ask_directive` call sites --
sequential, **panel**, parallel and free; "three" silently excludes the shipped
Compare & Decide preset.

**Board review** (`--board-review`, app: "Review the task board before work
starts" in the Conversation group, off by default and disabled outside a
Supervisor room): every wave of `[[TASK]]` directives — the first plan and
every rolling one — pauses in a card before `assign_workstreams` dispatches
it. Include/exclude, retitle, reassign, reorder, or send the whole board back
with a note the manager plans against. Silence is never approval, and a
board nobody answers stops the manager asking. Cheap live check:

```
python relay.py "create a file called one.txt saying ONE" --turns 2   --mode supervisor --board-review --moderator claude:claude-haiku-4-5:low   --agents claude:claude-haiku-4-5:low,claude:claude-haiku-4-5:low   --workspace <a throwaway folder> --no-brief
```

answer at the console prompt (or drop `1` / a refusal sentence into the
session's `say.txt`). Verified end to end 2026-08-27: approved once and both
files landed; refused once with "one task is not enough — also write a README
line", the note reached the transcript verbatim, and the manager re-planned
in the SAME run with exactly that second task added.
...
turns × seats. **`--unattended`** says nobody is at the console (Task
Scheduler, cron, a script), so an unanswered question is given up on after
at most `relay.ask_wait_limit` seconds instead of holding the run open
forever — the terminal's answer to the question `Run.background` answers
for the app. It is a DECLARATION, not a detection: there is no reliable
way to ask Windows whether a human is watching, and the tempting proxies
...
## Wave 2 — steering the run (2026-08-27)

- **W2.0 — run pinning, and it is the prerequisite the rest of this rests
  on.** `Api._webhook_on_start` spawned a bare `threading.Thread` and threw
  the Thread away, so `Run.is_running()` was False forever for that chat and
  it could be renamed, deleted, forked, re-continued or raced by a second
  webhook start while its seats were mid-turn. (The guards WORKED for chats
  this window started itself; they were blind to exactly the ones nobody was
  watching, and `RunManager.live()` had no caller at all, which is what kept
  the hole invisible.) `RunManager.spawn()` is now the ONE way a conversation
  thread comes into being and records it BEFORE starting it — a guard reading
  `is_running()` in between must not see a hole one instruction wide — and
  `live()` counts runs with a thread but no id yet, because a chat is live
  from the instant its worker starts and `adopt` happens seconds into the
  work. `_conversation(cfg, run=None)` / `_continue(cfg, run=None)` are
  PINNED: both used to write state, directory and identity through the
  `self._conv` / `self._session_dir` views, which resolve to whatever chat
  the WINDOW is showing. `adopt(run, chat_id, focus=False)` makes taking the
  focus pointer opt-in, `Api._emit_for` stamps every event a run emits, and
  `started` carries `background: true`, which the UI honours instead of doing
  `activeId = id; openTab(id)` unconditionally. `run_event_hook` takes
  AICHAT_SESSION from the FIRING run. Two more found on the way:
  **`adopt` never dropped a re-adopted run's old key**, so `_runs[old]` and
  `_runs[new]` became one object with `.id == new` and `open_session(old)`
  served the new chat's state — `fresh_stage()` stops it at source, because
  Josh typing into a reopened chat starts a NEW conversation while the python
  focus pointer still names the old one; and **`Run.unread` was a permanent
  zero** nobody incremented while the UI read `unread_count`, a key python
  never sent. `_thread`, `_staged_roles`, `_ask_lock` and `_chat_id` are gone
  too: a focused-run view with no caller is a focus leak waiting for its
  first one.
- **W2.1 — the queue dock.** Enter still SENDS; the ⏱ button (or Ctrl+Enter)
  HOLDS, in a dock above the composer where a row can be edited, reordered,
  dropped or sent one at a time. **The hold is client-side and that is the
  design**: the sequential loop reads `human_q` once per TURN — minutes —
  while parallel and free drain every 250 ms, so an edit against the engine
  queue would silently miss whenever the drain won the race and look perfect
  in exactly the two modes anyone would test it in. `HumanQueue` replaces the
  bare `queue.Queue` behind `Run.human_q`: same three methods, plus `qsize()`
  (published by `Api.jobs`) and `snapshot()`, and deliberately no edit and no
  drop. `Api.prepare_message` saves a queued row's attachments WITHOUT
  delivering it, so the row's text is exactly what will be sent — the bytes
  must go to the working folder, so a drop cannot unwrite them and the
  two-step delete SAYS "drop? (keeps 1 file)". The row is a TEXTAREA and the
  `[Josh attached a file: …]` lines are non-editable chips. Both `interject`
  and `prepare_message` refuse a leading `/` BEFORE writing anything, because
  four of the five drain sites tell a command from a message by that one
  test — and the fifth, `run_battle`, had no such test at all, so a plain
  interjection during a blind round was recorded as a command, answered
  "Unknown command /<first word>." and DESTROYED.
- **W2.2 — the board review**, and its MANDATORY rule is the state key:
  `state["board"]`, never `state["plan"]`. Plan Mode's key is written by
  `start_plan` with no mode check and it rehydrates, so a supervisor run can
  arrive already holding `{"phase": "drafting"}` — and since nothing on the
  supervisor path calls `approve_plan`, sharing the key would leave every
  seat read-only for the rest of the run. The gate runs BEFORE
  `assign_workstreams` (which rewrites `owner` and appends to `deps` as it
  dispatches), `supervise_next_wave`'s refusal REMOVES its in-place appended
  tasks (a task left pending keeps `plan_drained` False forever),
  `merge_board_edits` whitelists `brief` and `owner` only — eleven
  scheduler-owned fields survive, two of which (`replans`, `commit`) grant a
  repeat if blanked — and an owner comes back as the slot id's own TYPE,
  because a slot id is a number and `<select>.value` is a string. Dropping a
  task strips it from its dependents' `deps`: dropping is not editing, and a
  survivor pointing at an undelivered id is blocked forever. The gate has a
  DEADLINE (`board_abort`), or it reproduces the 2026-08-22 wedge where the
  clock, the spend cap and the watchdog are all checked at the barrier it
  blocks. Silence is never approval; a refusal is flagged (`board_declined`)
  rather than inferred from whether a note exists, and `plan_workstreams` is
  a thin loop over `_plan_once` because "send it back" has to MEAN something
  in the run Josh is watching — the auto-plan is a single pre-loop call and
  `supervise_next_wave` needs a non-empty drained board to get the floor.
  Live-verified end to end.
- **W2.3 — background jobs.** `Api.jobs()` is bridge-thread synchronous and
  takes NO conversation lock, ever: it is polled while runs are mid-turn.
  `turnClockText` is now the ONE clock rule the transcript's indicator and
  the jobs popover both call — `limit` is a real deadline and the only thing
  that earns "of X"; `idle` is a silence window and is not one, and in the
  app claude/gpt/ox ALWAYS have no limit. The popover measures silence
  against a real `lastact` the engine stamps on every activity event;
  feeding it the turn's START makes quiet equal the whole age, which paints
  every healthy long turn red. `Run.clocks()` is the one read path, under
  `Run.clock_lock` — and the CLEAR in `Api._rounds` takes it too, or the lock
  closes nothing against the one writer that removes entries.
- **W2.4 — `/objective` for any supervised run.** It shared a branch with
  `/checkin` and `/limits` gated on `continuous_on`, so a manager could only
  be re-aimed in an unattended run — exactly backwards, since Build Together
  and Keep Improving are the same workflow. Gated on `state["mode"] ==
  "supervisor"`, the test every other supervisor mechanism makes.
  `retarget_supervisor` is the one helper: `SUPERVISOR_MAX_WAVES` measures
  ONE goal, so a re-target that left the counter alone handed the manager a
  goal it had no waves left to pursue and `supervise_next_wave` returned
  "idle" at every barrier, silently, forever — and that shipped for Keep
  Improving too, because the continuous path resets only when the MANAGER
  picks the next objective. The watchdog's `replan` remedy had the identical
  gap. It deliberately does NOT re-plan or archive: it runs at a drain site,
  which in run_parallel is mid-round with seat threads alive. Also: the
  Master goal panel was fed `goal`, which is `meta["topic"]` — the opening
  message — for the whole conversation; the engine has published
  `supervisor_goal` all along and ui/index.html contained zero occurrences of
  the name.
- **The adversarial pass caught 17 real defects** across five lenses (33
  candidates, 33 refute-passes). The ones worth carrying forward as SHAPES:
  a "one read path" that does not cover the writer which REMOVES entries; a
  clock fed a substitute for the field it measures; a counter that persists
  its rows but not its id sequence; an error note living inside the container
  that is hidden exactly when the error fires; a card built only by the
  handler for one of its three events; a seed applied only at construction
  when every object is constructed empty first; and four tests that could not
  see their own subject. Full list in the commit.

## Wave 4 — proactive (2026-08-27). THE LAST WAVE.

Scheduled rooms: `schedule.py` + an in-app poller + `#schedModal`, over the
saved rooms that already existed. With this, every wave of the DeepSeek
Harness port is done — 0, 1, 2, 3, 4 and 5, plus the solo seat.

- **A schedule is a saved room, an opening message, and a recurrence.** The
  room carries everything else (seats, models, permission, the working
  folder, the whole orchestration recipe), and it is referenced by NAME, so
  editing a room edits every schedule that starts it. The `prompt` and the
  round count live on the SCHEDULE, because `roomCfgFromStage()` captures a
  stage, and a stage has neither.
- **The poller starts from `main()` and never from `Api.__init__`.** The plan
  said eighteen suites construct `app.Api()` directly; re-derived, it is
  **twenty-nine**, and a constructor-started scheduler would poll Josh's real
  `sessions/` folder inside every one of them and shell out to real CLIs.
  `_scheduler_tick(now=None)` is THE unit — the thread adds nothing but a
  clock — so every test drives it directly, and `Api.stop_scheduler` rides
  `main()`'s existing `finally`.
- **A missed window is skipped and SAID, never fired late**, and three days
  of downtime produce ZERO runs rather than three. `claim` advances the
  record BEFORE the run starts (`run_checkin`'s rule — a fire recorded
  afterwards repeats at every poll for as long as the conversation lasts)
  and is a compare-and-set, so one window is never claimed twice.
- **A fire SKIPS while any chat is live, and says so on the record.** The
  webhook RAISES there because a script is waiting for an answer; a schedule
  has nobody to answer to, and queueing would start an unattended room at an
  unpredictable hour. Different promise, deliberately, stated in the
  docstring rather than left to be inferred from the code.
- **The acknowledgement is per SCHEDULE and re-checked at FIRE time.** The
  plan named `permission == "full"`, `connectors` and `desktop == "full"`;
  the set also holds `browser == "full"`, `desktop == "allowlist"` (the rung
  that clicks WITHOUT asking) and an unbounded Keep Improving recipe. Rooms
  are saved by NAME and overwriting one is documented behaviour, so save-time
  validation alone would let a room gain Full access months after the box was
  ticked — the trap the plan does not mention, and the one this whole wave
  turns on. The tick box names the room AND the recurrence, and a change to
  either un-earns it.
- **`run_schedule_now` goes through the same `_launch_schedule` as the
  timer**, and does not consume the night's window. A second, friendlier code
  path is how a safety control acquires a way around itself.
- **The `scheduled` hook event needed the three edits the plan named**, and
  the third is now structural: `hookSave` walked `hookLabels`, so an event
  the BRIDGE knew about and the table did not rendered a row (the list comes
  from `r.events`) and was silently dropped on Save. It iterates the ids it
  PAINTED (`hookRowIds`) instead, and a test drives a row `hookLabels` has
  never heard of all the way to the bridge. The event fires for a skip and a
  miss too — a nightly room that did NOT run is exactly what a notification
  hook is for — and carries `manual` so a hook can tell a timer from a
  button press.
- **Live-verified twice, 2026-08-27.** A real poller thread fired a `once`
  schedule 20 s after its window, a real haiku seat wrote the file, the run
  was `background=True`, the focus pointer never moved, the `once` disarmed
  itself, the `scheduled` event fired, the user's event hook executed with
  the right detail, and the transcript carried "Started by the schedule
  'Smoke test' (Once, at …, room 'Scheduled smoke')". Separately, the real
  WebView2 window driving the real bridge saved both an innocuous and an
  acknowledged risky schedule, and refused the risky one until the box was
  ticked.
- **The live pass found two defects no token-free test could see**: the
  acknowledgement's sentence went stale the moment Josh typed a time (only
  the room picker repainted it, so a box ticked against "every day at 01:00"
  saved "every Mon, Fri at 02:30"), and a room at Full access did not admit
  that `relay.advisory_rung_note`'s ceiling applies — that a seat with a
  shell is not bounded by a desktop or browser allowlist.
- **The adversarial pass confirmed seven more** across five lenses: an
  over-claim in this module's own docstring (it credited naive `timedelta`
  arithmetic with DST-correctness it does not have), an edit that WIPED a
  schedule's history because the caller sends a form rather than a record, a
  spent `once` that could be re-armed into `enabled: true` with no next run,
  a launch that raised leaving the record on "starting…" with the window
  already spent, a save payload that hardcoded `enabled: true` (so editing a
  PAUSED schedule silently re-armed it), a `poll_seconds` the bridge
  published and nobody read, and `ack_covers` — an exported helper whose only
  caller was a test, deleted rather than dressed up as API.
- **63 rules RED-verified.** Four of the mutations were caught by NOTHING and
  were fixed as tests rather than as code: a narrowed `except`, an
  un-asserted announce, a mutation that turned out accidentally equivalent
  (`"" || every`), and a probe asserting a JS VARIABLE where the bridge
  consumes a PAYLOAD.

## After the waves — the leftovers (2026-08-27)

Not a wave. Wave 4 finished the staged port; this is the deliberately
out-of-scope list its handoff carried, and half of it is still deliberately
unbuilt (below).

- **An unattended `[[ASK]]` had no deadline — the one leftover that could
  WEDGE something.** `ask_abort` composed a deadline in continuous mode only,
  so a room started by a SCHEDULE at 01:00, or by the local webhook from a
  script, blocked its barrier until Josh pressed Stop in the morning — and the
  accumulated clock, the spend cap and the scheduled watchdog are ALL checked
  AT that barrier, so nothing could fire. Wave 4's `#schedModal` stated it as
  an unattended note; the engine now makes it true instead of merely admitting
  it. `relay.unattended(state)` reads ONE private bool, `_unattended`, which
  `app.py` sets from `Run.background`. Be precise about what that signal is:
  `Run.background` is a FOCUS policy — "never take the transcript Josh is
  reading" — set only by the webhook and by a scheduled fire, so it is the
  best available proxy and not a presence sensor, and `Api._mark_watch` is
  the ONE writer, called from the FOUR doors through which Josh proves he is
  there: `_conversation`, `_continue` (typing into a paused chat), `interject`
  (typing into a LIVE one — `continue_chat` refuses while a run is running,
  so without this a scheduled chat he joined mid-flight went on expiring the
  questions he was sitting there to answer) and `open_session`, which hands
  the run the very focus `background` exists to withhold. A plain bool rather
  than the Run, because relay must not learn an app type; PRIVATE like
  `_usage_io`, so `SessionStore.save`'s whitelist never persists it and a
  resumed chat is attended by construction — guarded by a source test that
  relay only ever READS the key, widened after an adversarial pass walked
  five write spellings past its first regex, the important one being
  `"_unattended": True` inside a dict LITERAL, which is how relay builds
  every other state key. **That guard was re-aimed on 2026-08-27, when the CLI
  gained `--unattended` and became the SECOND front end to declare this**
  (§ Closing the CLI's half). The rule it protects was never "relay must not
  write the key" but "the ENGINE must not": a front end is the only thing
  that can know how its own invocation started, so two of them each
  declaring for the run IT started is the `LoopIO` seam, not split
  authority. Four things fell out of it:
  - **THREE arms, not two.** The unanswered-question note's `else` arm names
    Keep Improving out loud, so a scheduled round-capped run falling into it
    would be told it is something it is not; and the attended arm ("Josh was
    unavailable") is equally wrong for a run he never opened. Nothing here may
    forge an answer, so all three are relay notes in the REQUESTER's queue.
  - **A note that states a boundary has to be rewritten when the boundary
    moves.** `schedule.unattended_notes` said "the run waits for an answer —
    only Keep Improving runs give an unanswered question a deadline". That
    sentence WAS the bug, written down and shipped as documentation. It now
    describes the deadline, and the number crosses as an ARGUMENT because
    schedule.py imports nothing from relay: `app.Api.room_risk` passes
    `relay.ask_wait_limit`'s answer FOR THAT ROOM, not the flat constant —
    a Keep Improving room's cap is its check-in interval, so "at most ten
    minutes" would promise twice what the engine allows. `ask_wait_limit` is
    therefore the ONE answer to "how long will an unanswered question wait",
    read by the engine and by the sentence Josh reads before arming a nightly
    room. Omitted, the note names no duration at all, because an unnamed wait
    beats a wrong number. `board_abort`'s docstring went stale in the same
    edit — it distinguished itself from `ask_abort` by "unlike it, this
    applies outside continuous mode too", which stopped being the
    distinction, and a docstring that defines itself by what a NEIGHBOUR does
    not do goes stale the moment the neighbour changes.
  - **The live run found the honesty gap no token-free test could.** The wait
    expired, the seat got its note, and the TRANSCRIPT said nothing: the note
    the seat is owed lives in `pending`, which is meta, which Josh never
    reads. `announce_lost_ask` — its twin, which records a question lost to a
    crash — has always persisted its notice; the question nobody ANSWERED did
    not. It does now, as a system row.
  - **The fourth is the arm's own wording.** "not holding for an answer and
    nobody replied" can both be false the moment Josh opens the chat and
    presses Skip; it says "started in the background with nobody watching,
    and no answer came before it moved on", which is true in every sub-case.
  Live-verified 2026-08-27 with a real poller, a real haiku seat and
  `ASK_WAIT_MAX` shortened to 60 s: `background=True`, `_unattended` True, the
  question emitted at 35 s, the wait expired at 60 s, `ask_pending` cleared,
  the requester handed the BACKGROUND note verbatim, no forged Josh row, the
  run finished on its own.

- **The digest ignored artifact descriptors, and that is W1.1's shape one
  consumer over.** `ORCHESTRATION_DESIGN_V2.md` § digests asks digest
  generation to prefer artifact descriptors over verbose file descriptions;
  `_digest_source` read only `message_id`/`name`/`text` while every seat row
  has carried a verified `artifacts` list since W1.1. `_digest_files_line`
  renders `files: path (N bytes), …` per row. **Measured before it was
  written**, because the trap is that the answer might be "almost none": 62 of
  1,118 rows across the real `sessions/` folder carry artifacts (11 of 51
  sessions), and in an 8-row window from `20260823-165418` **three of eleven
  produced paths were nowhere in the old source at all** — the prose that
  named them fell past the truncation point. Rules:
  - **It SPENDS from the same allowance.** `DIGEST_SOURCE_CHARS` exists
    because of the Windows ~32,767-char argv limit, so the files line comes
    out of the row's body allowance — and the subtraction lands AFTER the
    per-row `min(2400, …)` cap, not inside it: with a fat global `remaining`
    the min pins the body at 2400 either way, so the obvious spelling left
    the descriptors additive in exactly the common case (measured: one row,
    six descriptors, +186 chars). Turning this on must not grow a digest
    prompt at all. Real 8-row window: 12,024 chars before, 12,084 after,
    carrying ~370 chars of verified paths.
  - **Both truncations announce themselves.** A long reply keeps its
    `[truncated]` marker; a file list keeps `[+N more]`; and rows that never
    fit at all used to vanish in silence, which reads as "that was
    everything" — they now say `[N further messages omitted for length]`.
  - **Paths stay workspace-RELATIVE** (what the descriptor holds, what
    `read_text`/`read_image` take, what means the same thing to a seat whose
    cwd IS the workspace), and every field is treated as a claim, because
    rows come off disk: a non-list `artifacts` returns nothing rather than
    raising into a seat's turn, and a size that is not a count loses the
    SIZE, never the path.
  - **A seat cannot forge that line.** `DIGEST_PROMPT` certifies a `files:`
    line, and a body is inserted VERBATIM — so a reply containing one would
    reach the other seats carrying that certification and naming any path it
    liked, defeating the confinement `artifact_descriptors` exists to
    enforce. `_defuse_files_mark` indents a line-leading marker by one space,
    exactly memory.py's `_defuse` move for a heading inside a note, and the
    prompt names the POSITION (directly under a message header) rather than
    just the prefix. It also says what the line is NOT: a record of confined
    EDIT activity plus harvested images, so its absence is not a claim that
    nothing was produced.
  - **A row that does not fit is DEFERRED, not deleted.** `_digest_source`
    answers `(source, used)` and `deliver_hidden_digest` consumes only
    `source_ids[:used]`; progress is guaranteed because a chunk is appended
    before the budget is re-checked. Otherwise a length cap would do exactly
    what that function's docstring promises an OUTAGE may never do. The
    deferral is stated twice on purpose, to two different audiences: in the
    source so a summary cannot claim completeness, and appended to the
    delivered text by the relay itself, because a model told to be compact is
    the wrong keeper of a fact the seat needs.
  Live-verified 2026-08-27: three real haiku seats in a `--preset live-room`
  room, one addressing its reply with `[[TO:]]`; `notes.md` (90 bytes) really
  produced, three digest calls made, and the source handed to the real
  summarizer carried `files: notes.md (90 bytes)`.

- **Two cosmetic ones landed with them.** The Advanced drawer's two
  crowd-voiced `<option>` labels (`#budgetUnitSel`'s "Laps — everyone speaks
  once per lap" and `#completionSel`'s "The AIs themselves") repaint from
  `rosterChanged()` through `SOLO_OPTION_WORDS`, the same shape as
  `MODE_PRESETS`' `soloName`; the stored VALUES never change, because meta,
  replay, forks and saved rooms all read them. And the memory modal now
  FOLLOWS the chat: it was judged transient last time, and it is not —
  `Ctrl+Tab` / `Ctrl+1-9` / `Ctrl+T` switch chats straight through it (a modal
  here is not a focus trap), and `memAdd` sends `activeId` at CALL time, so
  the header could name project A while the note landed in project B.
  (`forget_memory` was already safe: the bridge refuses a scope the chat
  cannot see.) It repaints and says so rather than closing, keeping the draft
  in `#memText`.

- **Still deliberately unbuilt.** The board card stays a LIVE CONTROL, not
  history: a second wave replaces it and a reopened chat renders only the last
  board. That is not a bug — the durable record already exists in the relay
  row and in `supervisor_trace`, whose `board_reviewed` entries carry the
  WHOLE task list (`board_trace_tasks`), so a "past boards" view is a lens
  over data the engine already writes and belongs next to the Supervisor
  control log rather than inside the gate. Making the gate itself historical
  would blur a control with a log. Also unbuilt on purpose:
  `browser_mcp.WITHHELD`'s five performance tools, `MCP_DELIVERING_PROVIDERS
  = ("claude",)`, and `schedule.py`'s one-window assumption — all three are
  honest boundaries this repo states out loud.

## Closing the CLI's half (2026-08-27)

Not a wave either. The two live candidates the previous session left for
Josh to rule on, both of which he took.

- **`DIGEST_PROMPT` named the reader, and that was the whole defect.** The
  previous session had observed the summarizer answering back with
  clarifying questions and filed it as "harden it like `SUPERVISOR_PROMPT`
  rule 6". Measuring it first said something sharper. The old wording
  described the recipient as "one participant who was not directly
  addressed" — a DESCRIPTION, which sends the model looking for them in the
  source, where by construction they can never be, because being absent from
  it is the entire reason the digest exists. Two failure shapes fall out of
  that one cause, and neither is the one that was filed: on a thin packet it
  denies the premise ("No participant was excluded from these messages" —
  FALSE, and delivered into the excluded seat's own prompt under the relay's
  certification), and on a fat one it picks the only human it can see and
  writes "Relay Digest for Josh", "Awaiting your go-ahead", re-aiming at a
  seat the questions that had been put to Josh. `deliver_hidden_digest`
  knows exactly who the reader is (`state["agents"][i].name`) and was
  throwing it away — W1.1's shape a third consumer over. Rule 6's clause is
  still added, because it is what stops a model that still hesitates; it is
  the second half, not the first. Numbers and the live check are in the
  comment above the constant.
- **A third way a digest can be unusable is stated and NOT gated.** A summary
  that arrives, is not empty and is not a digest consumes the source rows
  with it, so unlike an outage it really does lose the peers' messages —
  which `deliver_hidden_digest`'s own docstring promises against. Measured at
  4 of 12 before and 0 of 12 after, so the fix is the prompt, not a detector;
  the docstring now says so rather than leaving an absolute claim standing.
  If it ever needs closing, the shape is a positive output contract the code
  checks (`moderator_pick`'s rule), never a blacklist of refusal phrases,
  which cannot tell a refusal from a digest that quotes one.
- **`--unattended` makes the CLI the second front end to declare it.** The
  guard that said "relay never WRITES `_unattended`" was protecting the right
  rule with the wrong subject: the ENGINE must not decide whether anybody is
  watching, but a FRONT END is the only thing that can know. `relay.main()`
  is `app.Api`'s peer, not part of the engine. `[[ASK]]` really is the only
  thing that waits forever in a CLI run — `board_abort` already bounds every
  run, the check-in's `permission` action is a wait Josh opted into, and
  `plan_gate` (which has no abort at all) has no CLI route into it, since
  `start_plan`'s only non-relay caller is `app.py`.
- **The source guard became an AST walker**, which is why it can now say
  WHERE the key is written rather than only THAT it is — the whole rule, once
  exactly one write is correct. It also gets statement-versus-mention for
  free. It is not strictly stronger though, and the adversarial pass caught
  the direction it lost: `ast.keyword.arg` is a plain `str`, so
  `state.update(_unattended=True)` produces no `Constant`, reaches no branch,
  and slipped past the fail-loud `else` as though it were nothing — a
  spelling the REGEX had caught. Whenever a check is rewritten in a stronger
  formalism, enumerate what the old one caught that the new one does not.
- **The adversarial pass found 10 distinct defects and not one was runtime.**
  Every single one was a test that could not see its own subject, or a number
  in a comment that did not match the data — including "3 of 3" where the
  data says 2 of 3, and "two real source packets" where one of the two is an
  invented fixture. Both are now stated as what they are. The lesson is where
  the risk actually sat: the code was measured and live-verified and correct,
  and the prose about it was where the errors were.

## The Changes tab and slash autocomplete (2026-08-29)

Two standing items from `docs/improvement-backlog.md` and
`docs/feature-ideas.md`, and the same shape twice: **the answer already
existed and had no surface.** Alloy has shelled git since Keep Improving
shipped and could not show a diff; the engine has known every slash command
since the first week and the composer could not name one.

- **THE DEFECT WORTH REMEMBERING: git resolves a repository by walking UP.**
  `status --porcelain` / `diff --numstat` / `log` run with `cwd=workspace`
  then answer for the WHOLE enclosing repo, with paths relative to its ROOT.
  The first version believed those answers described the workspace. Since
  Alloy's default workspace is `sessions/<id>/workspace/` INSIDE its own
  checkout, every ordinary scratch chat opened the tab and was shown Alloy's
  uncommitted source as "what changed in the working folder" — while the
  files its seats actually wrote were invisible (`sessions/` is gitignored,
  so status never lists them). Then the row click sent that root-relative
  path back, where it was re-rooted under the workspace, matched nothing, and
  rendered "No textual diff (binary file, or nothing changed)" — a positive
  false claim next to a row saying +3. On a NAME COLLISION it was worse than
  blank: the workspace's own `top.txt` was diffed and shown under the outer
  repo's `top.txt` row, two different files conflated with nothing saying so.
  `git_scope` is now the one resolver both readers start from
  (`rev-parse --show-toplevel` once), every git call is scoped by the
  workspace's own prefix, and every path crossing this boundary is
  workspace-relative in the same sense `read_text` means it.
  **37 tests passed while this shipped**, because not one of them put the
  workspace below the repo root. `NestedWorkspaceTests` exists for that.
- **`--literal-pathspecs`, and the reason a nested test could not see it.**
  `confine_to_workspace` treats git's pathspec magic as ordinary characters,
  so `":(top)top.txt"` passed containment and git then resolved it against
  the REPO TOP: measured, it returned the full working-tree patch of a file
  outside the workspace, and `":!ws.txt"` did it without naming a target at
  all. The flag turns the whole grammar off rather than blacklisting
  prefixes. But magic is only magic at POSITION 0 — `pkg/:(top)top.txt` is an
  ordinary filename — so a NESTED workspace defuses it by accident when the
  prefix is prepended, and the first version of that test passed with the
  flag removed. The case the flag protects is the workspace that IS the repo
  root, which is the main documented use. Test both.
- **One command rejects the flag: `check-ignore`** ("pathspec magic not
  supported by this command: 'literal'", rc 128, measured). Passing it there
  turned every ignored folder into an un-ignored one in silence — a control
  that does nothing, introduced by the fix for a different problem. Whenever
  a flag is applied uniformly to a family of subprocess calls, check each
  member accepts it; the failure mode is a wrong ANSWER, not an error.
- **A sentinel is not a sentence.** `{"git": False}` meant five different
  things — not a repo, git absent, the folder is gone, git refused the repo
  (dubious ownership / unknown format), a timeout — and the UI rendered all
  of them as "Not a git repository.", which is a measured claim about a
  folder git looked at and is simply false for four of the five. `git_scope`
  answers with a `reason`, and `refused` carries git's own sentence, which
  usually names the fix (`_git` had captured it on the merged stream all
  along and threw it away). `git_dirty` one screen up already made this
  distinction; a reader that has to write a sentence needs it more than a
  caller that can return None.
- **`git diff HEAD -- <spec>` exits 0 with no output for BOTH "unchanged"
  and "matched nothing"** (measured), so an empty patch as the catch-all
  answer states "nothing changed" about a request that failed. `tracked` is
  established FIRST via `ls-files --error-unmatch`, an untracked path that is
  not a file on disk is REFUSED, and `empty: "unchanged"` lets the pane word
  the two nothings apart. The old sentence also named a cause that cannot
  produce it: a binary file renders "Binary files a/x and b/x differ", never
  an empty patch.
- **An unborn HEAD needs `--cached`.** The fallback existed and used plain
  `git diff`, which compares the INDEX to the working tree — empty for a file
  that was just `git add`ed, i.e. exactly the state the branch was written
  for. A brand-new project's staged files all read "no changes".
- **A late answer must not repaint over a newer one.** Both bridge methods
  are worker threads with no ordering, and `scheduleFilesRefresh(300)` fires
  through the whole of an edit burst. `get_file_diff` echoes `path`/`sha`,
  `get_diff` echoes a `seq`, and `refreshDiff` holds one call in flight at a
  time (remembering a request that arrives meanwhile) — the Files lane's
  `filesBusy` discipline, which the Changes branch originally jumped by
  returning above the guard.
- **Assigning `#say.value` fires no input event**, so dictation left the
  slash menu holding rows painted for the pre-dictation text — and Enter then
  "completed" a command OVER the dictated sentence, destroying it silently;
  the natural second Enter sent `/clear`, wiping every seat's session. Fixed
  in `slashKey`, which re-derives from the current value, rather than only in
  the one caller that was found — `insertDictation` repaints too, so the menu
  is right before a key is pressed, but the guard is what covers the next
  writer.
- **A scoped command is SHOWN with its scope, never hidden.** `/ceiling`,
  `/checkin`, `/limits` and `/objective` only mean something in particular
  rooms; the UI does not know the room's state, the engine does, and it
  already refuses out loud. And **Enter on a name already typed in full falls
  through to send** — a menu that swallowed that Enter would make the
  most-used command feel broken.
- **`GATE_COMMIT_PREFIX` is spelled once.** `wave_gate` writes it and the
  Changes tab reads it back to draw ⛭; two literals would be the writer and
  the reader drifting apart in silence, so a test asserts the string appears
  exactly once in relay.py.
- The adversarial pass produced **17 confirmed findings across 7 distinct
  defects** (26 agents, every finding refute-verified). **15 rules
  RED-verified**, and the first pass of that verification found FOUR tests
  that could not see their own subject — each fixed by pairing the mutation
  with a test that actually exercises the rule, not by softening the rule.

## Checkpoint and rewind (2026-09-05)

feature-ideas #2. `wave_gate` has been committing every green wave since Keep
Improving shipped and the Changes tab has been showing those commits since
last week, so the whole feature was one missing verb over machinery that
already existed. Everything below was MEASURED against real git 2.51 before
it was written, and the two rules that matter most exist because the
measurement contradicted the design the backlog asked for.

- **"Refuse on a dirty tree" is useless, and that is the design's hinge.**
  The idea's own text says "refuse on dirty, show what would be lost", and it
  is wrong: the scenario this exists for is an overnight run that went red,
  and a red gate is exactly the state where the last wave's work is
  UNCOMMITTED. So a restore SAVES first — `git add -A -- <prefix>` then a
  partial commit under `SAVE_COMMIT_PREFIX` — and only then restores. A
  `git stash` would preserve it too and was rejected for one reason: Alloy
  has no terminal, so a stash is invisible in the only surface Josh has and
  recoverable only by typing `git stash pop` somewhere else. The commit
  appears one row above the restore, and the same button brings it back.
  Order is the whole safety story: the save happens FIRST, so a refusal there
  has overwritten nothing, and by the time anything is rewritten the work it
  would rewrite is in history.
- **It never moves HEAD.** `reset --hard` is wrong twice over — it rewrites
  the branch, and in the ordinary nested case (a working folder inside a
  bigger repo, which is EVERY default chat) it reverts the parent repo's
  other files too. A restore is a forward commit, so nothing leaves history,
  the undo is the same button, and the folder is left CLEAN — which matters
  beyond tidiness: `gate_commit` refuses when `dirty_at_start`, so a restore
  that left staged changes behind would silently disable the next run's
  checkpointing, and a second restore would refuse its own predecessor's dirt.
- **RC 0 IS NOT PROOF IT HAPPENED, and this is the finding to carry
  forward.** `git restore` only WARNS when it cannot unlink a worktree file:
  measured on Windows with the file held open by another process, it exits
  **0** with `warning: unable to unlink 'ws/x'`, the index entry gone and the
  old content still on disk as an untracked file. Commit that and git re-adds
  it from the worktree, `status` then reads CLEAN, and every surface —
  the card, the rail, the transcript note — states as measured fact that the
  folder was put back. So the POST-CONDITION is checked instead of the exit
  code, and the check is the preview itself, which is already the one reader
  that can see an untracked leftover: after a complete restore it answers
  `same` (verified both ways). `git diff --quiet <sha>` alone does NOT catch
  it — the leftover is untracked, so diff cannot see it either.
- **A restore can DELETE THE WORKING FOLDER.** Measured: `git restore
  --source=<sha> --staged --worktree -- <prefix>` is no-overlay, so restoring
  a nested workspace to a commit that PREDATES it removes every tracked file
  under the prefix and then the emptied directories — `ls` reports the working
  folder simply gone while Alloy is still pointing at it. `git cat-file -e
  <sha>:<prefix>` is the check, and it is a rev, not a pathspec, so there is
  no magic to fence.
- **`git diff` cannot see untracked files, and untracked files are the
  point.** A preview built from `diff --name-status` alone omits exactly what
  the save commit turns into tracked files and the restore then deletes. The
  `??` rows are merged in — and asked for with **`-uall`**, because git's
  default mode collapses a wholly-untracked directory into one `?? dir/` row
  while `git add -A` stages each file inside it (measured): the card promised
  "1 file" for an action that committed and deleted three. `git_overview` can
  live with the collapsed spelling because it is a LISTING; this list is a
  promise about an action, so it has to count what git will actually touch.
  Counts are SWAPPED at the same time: git answers in the sha→worktree
  direction and a restore is its inverse, so rendering git's own numbers
  would have signed every row backwards.
- **A merge in progress is refused, and the damage would be done by the SAVE
  rather than the restore.** Measured twice: `git add -A` RESOLVES a conflict
  by staging the marker text and discards stages 1/2/3, and the partial commit
  that follows then refuses outright ("cannot do a partial commit during a
  merge") — so without the guard the run reports `save_failed` ("nothing was
  touched") while `git ls-files -u` is already empty and `git mergetool` has
  nothing left to work with. Refused in the PREVIEW, so `git_restore` gets the
  check for free by re-running it.
- **A submodule is refused rather than described.** A restore moves the
  gitlink and cannot move another repository's checkout, so "revert" would be
  a promise nothing here can keep; `--recurse-submodules` is not the fix
  either, since it discards work inside a repo this function does not own.
  Gitlinks are mode 160000 in `ls-files --stage` (measured).
- **The prefixes are chosen so that none contains another.** An early draft
  spelled the restore commit `alloy: restore …`, which starts with
  `GATE_COMMIT_PREFIX` — so every restore would have worn ⛭ and claimed in
  the UI to be a verified wave checkpoint. `_commit_mark` is a lookup rather
  than an ordered chain, and the test pins the PROPERTY (no prefix is a
  prefix of another) rather than the order, because the order would be right
  by accident.
- **The busy gate looks wider than the chat, and the lock is per-folder.**
  `fork_session` refuses while ITS run is live; a restore checks EVERY live
  run, because two chats can be pointed at one project and the one that gets
  corrupted is the other one — and either folder containing the other counts,
  not just equality. A live run with no workspace yet is a chat seconds from
  having one and it blocks, under its OWN reason: the engine does not know
  where that chat is going, so "a conversation is running in this working
  folder" would be a measured-sounding sentence nobody measured.
  `_restore_lock` then serializes restores, because the card's button
  disables per-BUTTON while the corruption is per-FOLDER — clicking ⟲ again
  mints a fresh enabled one.
- **The way back is not always HEAD.** The card's safety sentence said
  "restoring to <head> puts it all back", and `head` is the last COMMITTED
  state — precisely the one commit that does NOT hold the uncommitted work
  the save step exists to keep. Naming it in both branches pointed Josh at
  the one recovery that throws that work away. The sentence now follows the
  same `save_total` test the commit count already followed. The feature's own
  test had encoded the right target all along (it restores to `r["saved"]`);
  only the sentence was wrong, which is this codebase's most common defect
  shape and the one the honesty lens exists for.
- **Live-verified in the real WebView2 window three times.** That pass found
  the defect the node harness structurally cannot: `#codeBody` is monospace,
  so the whole confirmation card — heading, file list and both sentences of
  safety wording — inherited it, while a CSS comment two lines up claimed
  only the path was. The stub has no cascade and reported the markup as
  perfect. (Same family as the `font:` shorthand and the sidebar button left
  out of the shared id-list rule.)
- **The adversarial pass produced 22 confirmed findings across 9 distinct
  defects** (36 agents, five lenses, every finding refute-verified; 31
  candidates). Two of the nine I had found independently before it ran, and
  the two highest-severity ones it found — the rc-0 unlink and the merge —
  were both re-measured here before being believed. **32 rules RED-verified**,
  and that pass found TWO tests that could not see their own subject: the
  rename test read only the verbs, so dropping `--no-renames` from the
  NUMSTAT call stayed green; and the half-applied-restore probe put the
  filename in git's `detail` as well as in `left`, so it could not tell which
  one rendered it. Both fixed by making the test exercise the rule, never by
  softening it.

## What happened while you were away (2026-09-05)

feature-ideas #18, the morning report. Everything below was MEASURED against
the real `sessions/` folder (52 metas) BEFORE a line was written, and the
measurement overturned four things the backlog and my own first reading
asserted.

- **The brief said "there IS no gate history". There is.** `supervisor_trace`
  carries `gate_result` entries in `running` → `passed`/`failed` pairs; the
  two-hour run `20260823-165418` holds 8 gates, 7 green and 1 red, with the
  red one's traceback in `detail`. My own first grep said zero only because I
  searched for type `gate` when the type is `gate_result` — the phase is
  `gate` and `supervisor_trace` maps phases to public type names. So the
  useful half of the gate scoreboard (#3) is aggregation, and needs no
  external output parsing at all.
- **`wave_report` and `current_objective` are NOT meta readers.** #18's own
  text names both as the report's sources. Both take a LIVE loop state:
  `wave_report` reaches `state["agents"][i].name` through `_seat_name`, and a
  reopened chat has no agents until `rehydrate` runs — which view-only chats
  never do. The card needed its own readers.
- **`supervisor_wave_index` is a CURSOR, not a counter**, and so is the
  trace's own `wave` field. See the gotcha; this is the single most important
  measured fact here.
- **`continuous.history` — `archive_objective`'s per-objective record — had
  no reader anywhere.** goal, task count, failures, the files verified on
  disk, and that objective's last gate, capped at 40. It existed only to be
  summarised into a memory note. W1.1's shape, one consumer over.

Two additive engine changes came out of it, both because a reader should
never have to parse its own side's prose: `wave_gate` stamps the checkpoint
sha as a `commit` FIELD (and `supervisor_trace`'s fact whitelist gained the
key), and `started_by` records WHO decided a run should happen.

- **`started_by` is the durable half of a question the engine already asks
  and throws away.** `unattended(state)` reads a PRIVATE `_unattended` bool
  that meta deliberately never keeps, so a resumed chat is attended by
  construction — right for the engine, useless afterwards, because a chat
  reopened at breakfast could not say whether it started itself at 01:00.
  FOUR front ends stamp it (`_conversation`, `_webhook_on_start`,
  `_launch_schedule`, `relay.main`), for the same reason only a front end may
  declare `--unattended`: the engine cannot know. An ABSENT record means "not
  recorded", never "josh", and the card words that as an inference.
- **`unattended` and `manual` are PROPERTIES of a start, not kinds of
  starter.** Folding either into `kind` would invent a who. `manual` is what
  keeps a Run-now press from being recorded as a 01:00 timer.
- **`fork.py` drops `started_by`**, beside the seat session ids it already
  clears: a fork is made by hand, now, and inherited provenance says a timer
  made a chat that did not exist when the timer fired.

**The adversarial pass ran six lenses and produced 56 candidate findings; the
refute phase died on a session usage limit, so every one was verified here by
hand against the real records, and 77 rules were RED-verified across three
passes** — which found EIGHT tests that could not see their own subject,
every one fixed as a test rather than by softening the rule. The confirmed defects worth carrying:
`was_interrupted` is TRUE for every LIVE run (its evidence is lifecycle
"active" with no termination reason — exactly a healthy run in progress), so
the card popped "While you were away" on a forty-second-old chat Josh had not
left; `record_usage` seeds `total_cost_usd: 0.0`, so a Gemini/OpenCode room
chipped "$0.00 spent" directly above a row reading "no cost reported";
`archive_objective` has TWO callers meaning opposite things and both wrote the
same record; the failing gate's tail was head-sliced, dropping the assertion
line; the trimming note printed `len(entries)` as if it were the retention
cap; a Build Together room was told "no verification command was configured"
for a stage only Keep Improving has; the card leaked into a brand-new
conversation (sendSay's sweep is `.msg`-only); "See what changed" painted into
a hidden rail; and `by_kind`, `outcome.trace` and `trouble.limits` were all
computed and never rendered — W1.1's shape three more times.

- **Live-verified in the real WebView2 window.** The card fits the feed, no
  horizontal scroll, ✕ / ≥ / → all render (measured against a
  known-missing-glyph width), the body inherits the UI font with only the
  failure tail and file paths monospace, and `--good` resolves to
  rgb(95,184,148) — a NEW variable, because the first draft tinted "this went
  well" with `--gpt`, which means A PARTICIPANT SPOKE.
- Also live-verified through a real CLI run: `--unattended` really lands
  `{"kind": "terminal", "unattended": true}` in meta and survives the save.

## Model Builder (2026-09-08)

The Collaborative Model Builder (built 2026-09-07/08 inside the OLD Tk checkout `C:\Alloy`, whose git history is unrelated to this app's; that checkout is `C:\Alloy-old` now and its history is branch `legacy-tk`) is a VIEW of this window: the cube button at the top of `#appNav` toggles `body.builder-open`, which hides every child of `<main>` except the nav and `#builderView` (a negation rule, not an id list). Josh's decisions for the port: in-app view (one window, one emit queue, the same test harnesses), core loop first (Milestone 2 = concept-approval panel, region drag, settings form), `legacy-tk` pushed, `docs/reports/e11-mast/` (his reference sheets) kept out of git in both repos.

- **Run model.** `builder_host.BuilderHost` owns ONE `BuilderSession` per process; the view's lifetime is not the session's (hiding the view never cancels a run; only app exit does, through `_builder_shutdown`). Every `Api.builder_*` method enqueues a job on the session's worker (or flips a thread-safe flag: pause, cancel, feedback, set_limits) and answers `{ok, job}` / `{ok: false, reason}` / `{error}` at once. Truth arrives as `builder` events: `busy`, `done`, `error`, `event` (an engine event), `snapshot` (the whole read model), `closed`, each stamped with `seq` and the host's `state`. The pump drains bursts and emits only the LAST snapshot of a burst (a 30-part project's snapshot is ~200-300 KB and `evaluate_js` is synchronous on the drain thread). The UI stores a snapshot received while hidden and paints it on open (`bDirty`).
- **No chat keys, above the gate.** Builder payloads carry no `chat_id`/`session_id`/`session`/`background`; the `builder` branch of `uiEvent` sits ABOVE the `payload.background && !chatId` drop and the not-my-chat gate, wrapped in try/catch so a view throw is shown in `#bJob`, never swallowed, and never breaks chat event delivery.
- **The bridge thread computes nothing.** `builder_snapshot` returns the host's cache of the last worker-published snapshot; `builder_overlay_rects` is pure over that cache; `builder_read_image` serves data URIs (thumbnail unless `full`) only from inside the OPEN workflow dir through `confine_to_workspace`, forbidden and missing being the identical `not available`; the native dialogs (`builder_pick_files`/`builder_pick_file`) use `webview.FileDialog.OPEN` (the older `webview.OPEN_DIALOG` prints a deprecation) on the bridge thread, which `pick_folder` already proved safe. An UNATTENDED `builder_start` consults `_plan_brake_verdict(probe=False)` (stored snapshot only) exactly like an unattended room.
- **Sessions are per screenplay.** `mock` (scripted seats) is fixed per `BuilderSession`, so the host re-creates the session when a different screenplay is requested (`builder_open(dir, screenplay)`, `builder_new_fixture` uses `harness.fixture_screenplay()`), refusing while a job is in flight. Scripted seats self-preflight; real seats REFUSE `start` without a current LIVE preflight report (R-18, R-21), and the view says so as the job's error.
- **Settings.** `sessions/builder.json` = `{"version": 1, "builder": {...}, "recent": [...]}`; only `builder` is read by `builder/config.py` (`.yaml` accepted by extension, lazy PyYAML; the package never discovers a host's config: explicit path, then `$ALLOY_BUILDER_CONFIG`, then `./sessions/builder.json`). Save deep-merges and replaces `presets` as a whole section; unknown keys are dropped on save; a corrupt file is defaults plus a warning, never a raise. Seat B on this machine points at `%USERPROFILE%\.codex\plugins\.plugin-appserver\codex.exe` (the npm codex on PATH is refused for `gpt-6-astra`); the gemini CLI is unsupported here. A project with no location anywhere lands under `sessions/builder-projects/<slug>`, never the repo root (`alloy-builder/`, `*.blend*`, `*.sqlite3*` are gitignored anyway).
- **Processes.** Provider CLIs and Blender are children of `builder/providers/process.py::run_process`: prompts on STDIN as packet files (`--add-dir <packet>`; never argv, the Windows 32,767-char limit), kill-on-close Job Objects with `taskkill /T /F` fallback and pid confirmation (unlike relay's `Agent.cancel()`, direct child only), `child_env` strips `CLAUDE*`/`ANTHROPIC*` exactly like `relay.clean_env` (a nested `claude` inheriting them reports "Not logged in": measured when Alloy itself runs from inside a Claude Code session). `restart.py` mid-build cancels the build; recovery runs on Resume, not on open (`needs_recovery` shows a notice); render pids are not tracked, so a killed app can leave one Blender render finishing. The builder's claude adapter uses `--output-format json`, so it never feeds `_plan_limits`.
- **The view (`ui/index.html`, the fenced BUILDER VIEW sections).** Full re-render per snapshot with `replaceChildren()`, selection kept by id, scroll positions kept; ALL builder DOM is built with `createElement` inside `bInit()` on first open (the node stub's static markup is flat, `closest()` is null, unknown members are truthy functions, and `byId` only knows STATIC ids, so probes walk the rendered grid); every bridge call is a literal `pywebview.api.builder_*(` wrapped in `bCall` (a settled promise, so a missing stub method never rejects unhandled); no interval is ever armed; the eleven Tk buttons plus New/Close/Recent/revision-0 go dark while a job runs and Pause/Cancel/Feedback/Apply limits never do (R-59). The Limits card always states what is typed versus what is in force (audit P2.5). Honesty strings are verbatim from the Tk window and pinned by `tests/test_builder_ui.py`: `STALE (<reason>): not current evidence (R-64)`, `no (generated: a hypothesis, never evidence of the original, R-32)`, `(unknown is never zero)`, `'Ready for user review' is never 'accepted'.`, `REASONING DOWNGRADE (reported, not silent)`, and the rest. New `#appNav` buttons go into the `#acctBtn, …` rule AND its hover twin; new `*Modal` ids go into both shared CSS lists, the Escape block, and `MODAL_CLOSERS` in `test_ui_boot.py`.
- **Chrome colours.** The builder has no participants: its chrome uses `--alloy`/`--muted`/`--text` only; the one permitted provider colour is a seat dot (`.b-seat[data-provider]`), per BRANDING.md's grammar.

## Hard-won gotchas (do not relearn these)

- **Three of those blind tests were blind in the same way, and it is the
  repo's oldest trap wearing new clothes.** One asserted the absence of a
  KEY when the rule was about a NUMBER, so publishing a wrong count under any
  other name stayed green. One searched a whole function for
  `"manual": bool(manual)` when `_launch_schedule` writes that string TWICE —
  removing the durable one left the transcript one, and the assertion was
  satisfied by the wrong occurrence. One drove `loadReport(id)` with `id`
  equal to `activeId`, so `session_report(id)` and `session_report(activeId)`
  were the same call and the rule was invisible. A fourth checked a cap
  signal with a fixture that also tripped the structural signal; a fifth
  asserted an unknown-chat refusal with no focused run for a fallback to fall
  back TO. In every case the test named the right rule and could not observe
  it — which is exactly why the mutation pass exists, and why a green suite
  is not evidence that a rule is guarded.

- **A wave NUMBER is a cursor, not a counter, and neither is the trace's copy
  of it.** `plan_workstreams` sets `state["supervisor_wave_index"] = 1` on
  every successful plan, and `supervisor_trace` stamps that value onto each
  entry. Measured on the real two-hour run `20260823-165418`: the wave field
  runs 1,1 → 2,2 → **1** → 3,3 → 4,4,4 → 5,5 → **1**,1,1, six plans were
  dispatched, and the meta ends reading `supervisor_wave_index: 1`. So both
  `meta["supervisor_wave_index"]` and `max(entry["wave"])` under-report, in
  the direction that erases the whole night. The honest counter is the number
  of `plan_created` entries — emitted from exactly ONE call site. The same
  fact is why `supervisorWaves()` renders that run's control log as Wave 1,
  Wave 2, Wave 1, Wave 3, Wave 4, Wave 5, Wave 1: duplicate, non-monotonic
  labels, from a comment arguing that cutting on the engine's number beats
  cutting on `plan_created`. Left alone deliberately — a re-plan really does
  start a new wave 1 — but do not read those labels as a count.

- **Before counting entries in a log, count the CALL SITES that write them.**
  The rule is not "is this event interesting" but "does exactly one
  occurrence of the thing produce exactly one entry". Measured per phase in
  relay.py: `plan_created`, `run_revived`, `limit_reached`, `goal_accepted`
  and `goal_unresolved` have one site each; `supervisor_error` has fifteen
  but one entry per failure, so it still counts; `health_check` has EIGHT and
  emits 2-3 entries per check-in, and `objective_set` has FOUR and several
  per rollover. Those last two cannot be counted at all, and the alternative
  — matching our own entry TITLES — is one wording change from silently
  becoming wrong. A number that cannot be derived honestly is not published.

- **`status="passed"` on a bookkeeping entry inflates every count of
  passes.** `wave_gate` traced its commit-binding note with the same status
  as a real verdict, so counting gate outcomes by status counted one extra
  green per CHECKPOINTED wave — the count drifting further from the truth the
  better the run was doing. It is `status="committed"` now. Whenever one
  function writes several entries into one log, ask whether a reader can tell
  its verdicts from its notes without reading prose.

- **`was_interrupted(meta)` is TRUE for every run in progress.** Its evidence
  is `lifecycle == "active"` with no `termination_reason`, and that is
  exactly what a healthy live run looks like — the distinction it was written
  for only exists once the process is gone. Any reader that means "this
  crashed" must pair it with a liveness check; without one, pressing Send and
  switching tabs popped a "While you were away" card on a forty-second-old
  conversation, and did it again on every switch.

- **`record_usage` SEEDS `total_cost_usd: 0.0` on its first call**, whatever
  that turn reported. So the total's mere presence is not evidence that
  anything was measured, and reading it as evidence chipped "$0.00 spent"
  above a spend row saying "no cost reported" — the two halves of one card
  contradicting each other, inside a feature whose stated rule is that a
  number nobody reported is blank. Evidence is a real per-seat or per-kind
  cost; anything else is the seed.

- **One function retiring a board for two opposite reasons wrote one
  record.** `archive_objective` is called by `next_objective` (the manager
  judged the goal FINISHED) and by the check-in watchdog's `replan` remedy
  (the board is STUCK). Both appended the identical `continuous.history`
  entry, so every later reader counted an abandoned objective as a met one —
  including `describe_objective`, which wrote "Objective met: …" into Alloy's
  own MEMORY, where a later run reads it back as fact. It takes an `outcome`
  now, and a record written before that is honestly unknown rather than
  assumed met.

- **A span is not a duration, and the tempting one is always available.**
  Three times in one module: `created`→`updated` measures how long the CHAT
  existed (72,949s on a real session worked on across two days), the control
  log's first→last bracket includes every idle gap between its entries
  (39,307s across ELEVEN entries on `20260822-220535`, for a run that worked
  minutes) AND shrinks as the log is trimmed, and only Keep Improving's
  accumulated `elapsed_s` is time actually spent working. The first draft
  promoted the middle one to the printable duration under the label
  "active" — the same mistake the docstring rejected the first one for, one
  field over. Publish spans as ranges, and let exactly one field be callable
  a duration.

- **A truncation note that quotes `len(entries)` states the wrong cap.**
  It coincides with the real cap only on the path where the cap itself
  detected the trim; on a structural signal it told the reader Alloy keeps
  three control-log entries — a false claim about the product, made inside
  the one sentence whose entire job is to be trustworthy.

- **A git command's exit code is not its post-condition, and `git restore`
  proves it.** It exits **0** when it cannot unlink a worktree file, leaving
  only `warning: unable to unlink 'x'` on stderr — measured on Windows with
  the file held open by another process (an editor, a scanner, a dev server a
  finished room left running). The index records the deletion, the old file
  stays on disk untracked, the next commit re-adds it from the worktree, and
  `git status` then reads CLEAN, so no later reader can see the gap. Whenever
  a git call is the load-bearing step of an action you then REPORT, check the
  post-condition rather than the return code — and check it with something
  that can see untracked files, because `git diff` cannot.

- **A "read-only" panel that grows one writing verb makes every sentence
  about it false.** README's Changes-tab section ended "Everything here is
  read-only: Alloy never commits, stages or reverts from this tab" — a true,
  deliberate promise that this feature broke, and exactly the line nobody
  re-reads while adding a button. When a surface gains a capability, grep the
  docs for what they promised about the SURFACE, not just for the feature
  name.

- **`git restore --source` will delete a directory, including the one you are
  standing in.** Everything else in this repo that touches the workspace
  either writes inside it or reads it; this is the first call that can remove
  it, and it does so silently with rc 0 (measured: restoring a nested
  workspace to a commit predating the folder left `ls: cannot access 'ws'`).
  Whenever a git command is scoped by a pathspec, ask what it does when the
  pathspec matches nothing in the SOURCE — the answer is not always "nothing".

- **`git add -A -- <prefix>` is load-bearing before a partial commit.**
  `git commit -m X -- <prefix>` does NOT pick up an untracked file inside the
  pathspec: it exits 1 with "no changes added to commit" (measured). So the
  step that exists to preserve uncommitted work is exactly the step that drops
  it if the `add` is read as tidiness and removed. Its OWN pathspec is
  load-bearing too, and a test cannot see that unless something outside the
  folder is UNTRACKED — with the outer file already staged, dropping the
  pathspec changed nothing observable and 47 tests stayed green.

- **`git status --porcelain` collapses a wholly-untracked directory into one
  row while `git add -A` stages every file inside it.** So a list built from
  the default status under-reports an action's blast radius by however many
  files are in that folder. `-uall` is the flag; the rule is that a LISTING
  can live with git's own aggregation while a PROMISE ABOUT AN ACTION has to
  count what git will actually touch.

- **`git add -A` resolves a merge conflict.** It stages the conflict-marker
  text and discards stages 1/2/3, after which `git mergetool` and
  `git checkout --ours/--theirs` have nothing to work with and only
  `git merge --abort` is left. Measured, and worth stating because the
  obvious reading of "add everything and commit it, so nothing is lost" is
  the exact opposite of what happens mid-merge. Anything that stages a whole
  folder must first ask whether that folder has unmerged paths.

- **A fixture pinned to an absolute instant is a time bomb, and it went off
  mid-session** (2026-08-29). `tests/test_plan_limits.py` carried a REAL
  captured payload whose `resetsAt` was 2026-08-29 18:00. The bridge tests go
  through the shipping `get_plan_limits` / `_plan_brake_verdict`, which ask
  `plan_limits.worst(snap)` with the REAL clock because that is what
  production does — so at 18:00 that day the reading expired, `worst`
  correctly answered None, the brake correctly stopped refusing, and three
  tests began failing PERMANENTLY (the instant only recedes further). The
  full suite passed at 16:20 and failed at 18:40 with no code change between.
  The measurement is kept exactly as captured — the parsing tests still pin
  every field and pass their own `now` — and what is now anchored to the real
  clock is the LIVENESS, which is the property those tests are about; the two
  expiry tests derive "past its reset" from the reading instead of from a
  constant. Verified by running the suite with `time.time` shifted a year
  ahead. **Whenever a fixture's meaning depends on the wall clock, build it
  relative to the clock**, and be suspicious of any suite that starts failing
  with no diff behind it.

- **A deadline that exists in one mode is a control that does nothing in the
  mode nobody is watching.** `ask_abort` capped an unanswered `[[ASK]]` in
  continuous runs only, and the reasoning was sound at the time: Josh armed a
  Keep Improving run to keep going without him, so only that run had opted
  into moving on. Then scheduled rooms shipped and produced a SECOND way to be
  unanswerable that nothing had been taught about, and the note in
  `#schedModal` documented the hole rather than closing it. Whenever a control
  is gated on ONE way of reaching a state, list the other ways of reaching it
  — and when a new one arrives, grep for every gate that names the old one.
  A THIRD way arrived on 2026-08-27 and was the same shape: `relay.py` from
  Task Scheduler is genuinely unattended, and the fix for scheduled rooms
  had taught the engine only about `Run.background`, an APP fact. The list
  of ways to reach a state is never finished; what can be finished is the
  habit of asking who ELSE can reach it.
  Its sibling drifted the same day: `board_abort`'s docstring distinguished
  itself with "unlike `ask_abort`, this applies outside continuous mode too",
  and a docstring that defines itself by what a NEIGHBOUR does not do goes
  stale the moment the neighbour changes.

- **A self-referential assertion cannot see a template regression.** Guarding
  the digest prompt's five `{reader}` slots, the obvious "derive the expected
  text from the template itself" (`DIGEST_PROMPT.format(source="",
  reader=name)` and assert the real prompt starts with it) looks strictly
  better than counting occurrences. It is blind: a sweep that removed each
  slot in turn changed the template AND the expectation together, so it
  matched happily all five times. It does still catch a CALLER bug (a wrong
  name, a spliced source), so both halves are kept and the comment says which
  guards which. The count it replaced was worse in a different way — a floor
  of 4 against 5 slots, taken over the seat-authored SOURCE as well, so a peer
  who happens to name the reader three times clears it with four slots gone.

- **`"--flag" in help_output` matches `--flagX`.** The `--help` half of the
  `--unattended` test was RED-verified by renaming the flag, and stayed
  GREEN — the rename left `args.unattended` an `AttributeError` waiting to
  happen and the substring sailed past. `assertRegex(out,
  r"--unattended(?![\w-])")`. The wrap-token family again, and note where it
  was caught: not by reading the assertion, but by mutating the thing it
  claimed to test. A substring test on an option, an id, a class name or a
  path is a prefix test until proven otherwise.

- **An `assertIn` on a source segment sees the name and never the meaning.**
  `assertIn("args.unattended", wired[0])` passed while the state key was
  `not args.unattended` (every console run expiring its questions, the flag
  itself wedging), while it was `bool(args.unattended) and False` (the flag
  inert, the wedge restored), and while argparse said `store_false`. All
  three were green across the whole 2776-test suite. `assertEqual` on the
  exact segment, plus an AST check of the `action=`.

- **An adversarial-review agent that mutates a file to check a test will
  restore the copy it read, and that silently reverts whatever you wrote in
  between.** Lost one commit-sized edit to it on 2026-08-27 — the change was
  applied, its suite passed, and half an hour later the function was back to
  its previous body with no error anywhere. The tell was a test failing on an
  assertion that had passed minutes earlier. Snapshot the working tree to the
  scratchpad before launching a review that is allowed to mutate, do not edit
  tracked files while one is running, and diff against the snapshot when it
  ends. (This is the same shape as the two-Alloy-window `claim` race the
  scheduler documents, one tool layer up.)

- **A modal is not a focus trap here, so "the modal reads `activeId` when it
  opens" is not a cosmetic staleness.** `Ctrl+Tab` and `Ctrl+1`-`9` and
  `Ctrl+T` all change the chat with the memory modal open, and every action in
  that modal sends `activeId` at CALL time — so the header could name project
  A while `memAdd` wrote the note into project B, with nothing saying so. The
  `forget_memory` half was already safe only because the BRIDGE re-checks the
  scope against the chat. Whenever a panel is "about the current X", ask what
  changes X behind it, and whether its actions read X at open time or at click
  time; disagreeing on that is the bug, not the staleness.

- **The stub's selector engine ignored the descendant combinator, and two
  of the UI's own selectors had been returning `[]` in the harness for
  months** (found 2026-08-27 writing Wave 4's probe). `matches(el, sel)` split
  on commas and matched each part as ONE compound, so `"#schedDays
  .sched-day"` took the `#` branch and compared `el.id` to the whole string —
  no match, no error, nothing. `ui/index.html` was already using that form
  (`"#feed .msg"`, `"#battleBar .vote-btn"`), so those two had been silently
  answering nothing in every run of `test_ui_boot` while working perfectly in
  a browser, and a Wave 4 probe that read the day checkboxes would have
  reported the product broken. The engine now walks `parentElement` for the
  earlier compounds. Same family as the no-op `classList`, the permissive
  Proxy, `replaceChildren` and the reflectionless `dataset`: **whenever a
  probe cannot see something, ask whether the STUB is answering before
  concluding the product is fine** — and the corollary this one adds, that a
  stub limitation is not confined to the test being written, because the
  product may already depend on the thing the stub cannot do.

- **A test that pins a literal SELECTOR STRING breaks on the next entry and
  says nothing about the new one.** `test_the_memory_modal_is_registered_in_all_three_places`
  asserted `"#statsModal, #memModal {"`, which is one modal-id list wrapping
  differently — so adding a THIRTEENTH modal failed a test whose subject was
  the twelfth. It now derives the ids from the markup (`<div id="…Modal">`),
  slices each rule's selector list exactly, and checks all three places for
  every one, with a hand-kept `MODAL_CLOSERS` map that fails LOUDLY on a
  missing entry rather than skipping it. Same move
  `test_every_sidebar_button_is_in_the_shared_style_rule` already makes, and
  the next modal is now covered for free.

- **A "run now" button on a scheduler is a second code path around a safety
  control unless it IS the first one.** `Api.run_schedule_now` calls the same
  `_launch_schedule` the timer calls, so the acknowledgement re-check, the
  missing-room refusal and the busy skip are one implementation. The only
  thing it does differently is not touch the clock — and that difference is
  invisible to a test unless the schedule is ALREADY DUE, because for a
  future window `claim` computes the same next occurrence either way. A
  mutation that claimed the window passed the first version of that test.

- **A hook event that is not a conversation event must not inherit the chat
  on screen.** `run_event_hook` falls back to `self._runs.focused()` when a
  payload carries no chat id — right for a conversation event that lost its
  stamp, and W2.0's exact bug for `scheduled`, which carries none BY DESIGN.
  Measured before the fix: `AICHAT_SESSION="some-other-chat"`,
  `AICHAT_TITLE="the chat Josh is reading"`. A script acting on that acts on
  the wrong conversation, which is strictly worse than acting on none.

- **An acknowledgement is a SENTENCE, and the sentence has to follow the
  form.** Alloy's schedule modal paints "I understand this starts <room>
  <recurrence>, unattended, …" — and only the room picker repainted it, so a
  box ticked against "every day at 01:00" saved "every Mon, Fri at 02:30".
  Every field the sentence quotes repaints it now, and a sentence that
  CHANGES after the tick un-earns it and says why (an identical repaint must
  not, which is its own rule). Found by driving the real WebView2, not by
  reading: the node harness has no notion of which handlers a page binds
  until something calls them.

- **An edit sends a FORM, not a record.** `save_schedule` replaces by id, and
  the first version let `normalize` read `runs` / `misses` / `last_run` /
  `created` out of the incoming form — which the UI does not send. Measured:
  changing a nightly job's time reported it as having never run. The store
  carries those fields over from the row it replaces, because only the store
  has them.

- **A stub that lies is worse than one that refuses — three more of them**
  (2026-08-27, all found by RED passes over Wave 2). `tests/test_ui_boot.py`'s
  DOM stub answered unknown members with a no-op function via a permissive
  Proxy, and three standard members were unknown: `replaceChildren` (so every
  list in the UI that rebuilds itself appeared to GROW on each repaint, and
  four dock tests were reading that as the product's behaviour), element
  `addEventListener` (so every keyboard binding in the page was structurally
  untestable — a probe could only call the handler directly, which passes
  whether the key is bound or not, and breaking Ctrl+Enter left the suite
  green), and `parentElement` (so `x.parentElement.children` was `undefined`,
  not an error anyone could read). `matches` also ignored `.class[attr]`
  entirely, so a renderer finding its own card built a new one every repaint;
  and `dataset` was a plain object reflecting to no attribute, so
  `dataset.activeBoard = "1"` could never be found by that selector. All five
  are implemented for real now. The rule generalizes: whenever a probe cannot
  see something, ask whether the STUB is answering, before concluding the
  product is fine.

- **`turnClockText`'s `lastact` is the field it measures, not a stand-in for
  it.** The jobs popover passed the turn's START time in that slot, which
  makes `quiet` identical to the whole age — so every healthy background turn
  past half its idle window rendered red "quiet M:SS of 5:00", from inside the
  function written to prevent exactly that lie, while the transcript's
  indicator for the SAME seat stayed neutral. In the desktop app the turn cap
  is CLI-only, so claude, gpt and ox ALWAYS have `limit` None and only the
  silence branch is reachable: this is not an edge case, it is the normal
  case. The engine stamps `lastact` on every `activity` event now. General
  form: when a renderer has no value for an argument, pass nothing and let
  the renderer withhold the claim — never substitute a different measurement.

- **A refusal that saves first cannot un-save.** `Api.interject` wrote the
  attachments and THEN refused a leading `/`, leaving files in the workspace
  with no message that names them; its twin `prepare_message` had always
  checked first. Any validation that can refuse belongs above the first side
  effect, and when two functions do the same job they must do it in the same
  order.

- **An error note inside a container that hides itself is invisible exactly
  when it fires.** `#queueNote` lived inside `#queueDock`, and the dock is
  hidden whenever the queue is empty — which is when all three of the dock's
  refusals happen. The empty-queue branch also cleared the note it had just
  written. Whenever a control reports a refusal, check that the refusal's
  home is visible in the state the refusal leaves behind.

- **`Api.stop` with an id it did not recognise stopped the chat on screen.**
  `_resolve_chat` exists so that "an unknown chat_id is REFUSED, never
  silently applied to whichever run happens to be focused — stopping the
  wrong conversation is worse than not stopping at all", and the fallback one
  line under it did precisely that. A NAMED unknown chat is refused now; only
  a BARE press falls back to the focused run. Its "interrupted N seats"
  notice went out unstamped too. The shape: a "still honour the press"
  fallback must distinguish "you did not say which" from "you said one I do
  not have".

- **A store whose own format is markdown can be split by the content it
  stores** (Wave 3, 2026-08-27). `memory.py` starts an entry at `## <id>`, so
  remembering `"Findings so far:

## Results
It is fast."` wrote ONE note
  and read back TWO -- the second with id `Results`, credited to nobody, and
  a `/forget` on the original id removed only the first half. Nothing raised
  and the round trip "worked"; only a test that counted the entries could see
  it. The fix is one leading space on write (`_defuse`), chosen because
  CommonMark allows up to three spaces before a heading, so the file renders
  identically to a human while `^##` no longer matches -- and the write SAYS
  it indented a line rather than changing the text silently. The general
  lesson: whenever a serialization format has structural characters, ask what
  happens when the DATA contains them, and check by counting records out, not
  by eyeballing the file.

- **Free mode's ask handling is CONDITIONAL, and the other three loops' is
  not** (Wave 3, 2026-08-27). Wiring a new per-reply handler in beside
  `handle_ask_directive` at all four call sites looks like a mechanical edit
  and is not: `run_free` wraps its call in
  `if any(name == "ASK" for name, _ in peel_directives(reply)[1])`, because a
  free-mode ask blocks a seat's own thread while its peers keep talking. Drop
  a handler inside that branch and it fires only when the SAME reply also asks
  Josh -- so `[[RECALL]]` worked in three modes and silently did nothing in
  Talk Live. The suite caught it because it drives all four loops by name and
  compares them, not because the insertion looked wrong. Same family as the
  four seat floors and the four ask call sites: **enumerate the sites and read
  each one's guard, never pattern-match the line above.**
...
- **One loop now** (was: two duplicated loops, extracted 2026-08-16):
  ...
  tests — see `tests/` (2943 tests across 81 suites, all runnable as plain scripts, or `python tests/run_all.py` for the lot).
...
- **A seat's `[[ASK]]` can wedge an unattended run, and every brake is
  ...
  composes the caller's abort with a deadline whenever nobody CAN answer, and
  both front ends already poll `abort` and return `None` from it, so no
  front-end change was needed. The expiry takes the documented unanswered
  path — a relay note in the requester's queue, never a forged answer. A
  `permission` check-in is NOT subject to this: Josh explicitly configured
  that one to wait.
  **There are TWO ways to be unanswerable and the first fix only knew one.**
  It was continuous-mode-only until 2026-08-27, which left a room started by
  a SCHEDULE or by the webhook wedging exactly as described above. See
  § "After the waves" below, and `relay.ask_wait_limit`, which is now
  the one answer both the engine and the modal read.
...
- **Event hooks** (#16): user shell commands on `question`/`checkin`/`done`/
  `gate_red`/`scheduled`. Config beside tabs.json (`read_event_hooks`/`write_event_hooks`,
...
**Token-free first**: `tests/` holds 2943 tests across 81 suites (plus three
...
Scheduled rooms (no tokens): `python tests/test_schedule.py`. The LIVE
check has no CLI form -- schedules exist only in the app -- so it is a
headless `app.Api` with a real seat: redirect `relay.SESSIONS_DIR`/`TABS_FILE`/
`ROOMS_FILE`/`MEMORY_DIR` and `app.SESSIONS_DIR` at a temp dir, `relay.save_room`
a one-haiku-seat room pointed at a throwaway workspace, `api.save_schedule` a
`once` due ~40 s out, `api.start_scheduler()`, and wait. Verified 2026-08-27:
fired 20 s after its window, wrote its file, ran `background=True` without
moving the focus pointer, disarmed itself, fired the `scheduled` event and the
user's event hook, and said so in its own transcript. The UI half is the same
shape with a real `webview.create_window(..., js_api=api)` and `evaluate_js`
driving `openSched()` -- and note that `evaluate_js` does NOT await a Promise,
so an async probe has to park its result on `window` and be polled for.
...
Checkpoint and rewind (no tokens, no CLI): `python tests/test_restore.py`.
There is deliberately no CLI flag for it — the functions ARE the API, so the
live check is a `python -c` against a THROWAWAY repo (never this one):
`python -c "import relay,json; print(json.dumps(relay.git_restore_preview(r'<ws>', '<sha>'), indent=1))"`,
then `relay.git_restore(ws, sha)`. The UI half needs a real window: point a
`view_workspace` at the repo, `railTabSet('changes')`, click the row's `.crst`
button, and click `#codeBody .rst-go` — allow SEVERAL seconds after the
click, because a restore runs the whole preview twice (once to plan and once
...
Model Builder (no tokens): `python tests/test_builder_suite.py` runs the pytest suite
(`tests/builder`, ~270 tests, ~100 s) with `blender` and `live` deselected; the app-side suites are
`python tests/test_builder_host.py` (host, fake runner + scripted seats), `python tests/test_builder_bridge.py`
(headless `Api`, fake window: every `builder_*` answers on the bridge thread with no subprocess, packets carry
no chat keys, the unattended brake is consulted with `probe=False`, images are confined),
`python tests/test_builder_ui.py` (source contracts), and the `BuilderViewUiTests` class of
`test_ui_boot.py` (the real `uiEvent` fed `tests/fixtures/builder_snapshot.json`, a fake-runner fixture run to
its review gate; regenerate it with the same recipe when the read model changes). The real-Blender layer is
