...
- **The seats have DIFFERENT capabilities, and only the preamble can say so.**
  ...
  too (`generate_image` in its tool list) but writes them to
  `~/.gemini/antigravity-cli/scratch` — agy IGNORES the process cwd for file
  writes and then reports the file as being "in the current working
  directory", so the image never reaches the shared folder (tested
  sandboxed AND yolo); **claude has no image tool at all** (checked its full
  ...
  `codex features list`; any probe failure ⇒ {} ⇒ claim nothing), and
  Gemini deliberately does NOT offer itself for image work because an image
  the others cannot see is not a routable capability. `preamble` renders the
...
**Token-free first**: `tests/` holds 246 tests, each suite a plain script
