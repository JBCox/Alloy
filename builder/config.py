"""Builder configuration (spec D9): the top-level ``builder`` key of a settings file, parsed leniently.

The settings file is ``.json`` (the app keeps it at ``sessions/builder.json``) or ``.yaml``; the mapping
under ``builder`` has the same names either way. A missing key means defaults. Unknown sub-keys are kept
in ``raw`` and ignored. Values of the wrong type fall back to the default and are reported in
``warnings`` so the CLI and the app can show them. This package never discovers a host application's
configuration: the host passes the path (``load(path)``), the CLI launcher sets ``ALLOY_BUILDER_CONFIG``,
and running from the app's root finds ``./sessions/builder.json`` on its own.
"""
...
ENV_SETTINGS_PATH = "ALLOY_BUILDER_CONFIG"     # the only discovery the package does: an explicit path from its host
DEFAULT_SETTINGS_RELPATH = Path("sessions") / "builder.json"   # the app's layout, relative to the cwd it runs from
...
class BuilderConfig:
    ...
    def load(cls, config_path: str | Path | None = None) -> "BuilderConfig":
        """Settings from a ``.json`` or ``.yaml`` file. Discovery order: the explicit path, else
        ``$ALLOY_BUILDER_CONFIG``, else ``./sessions/builder.json`` when it exists. No file means defaults;
        a malformed file raises ``ValueError`` (a typo is reported, never silently replaced by defaults)."""
        raw: str | Path | None = config_path
        if raw is None:
            raw = os.environ.get(ENV_SETTINGS_PATH) or None
        if raw is None:
            candidate = Path.cwd() / DEFAULT_SETTINGS_RELPATH
        ...
        if not path.is_file():
            return cls.from_dict({})
        return cls.from_dict(read_settings_file(path))
...
def read_settings_file(path: str | Path) -> dict[str, Any]:
    """The whole mapping of a ``.json`` or ``.yaml``/``.yml`` settings file (``{}`` when empty). Only the
    ``builder`` key is meaningful to ``BuilderConfig``; a host may keep other keys beside it."""
    path = Path(path)
    ...
    if suffix == ".json":
        ...
        import yaml  # optional dependency: only YAML settings files need PyYAML

        data = yaml.safe_load(text) or {}
    else:
        raise ValueError(f"{path}: a builder settings file must be .json or .yaml")
