...
class BuilderSession:
    ...
    def _open_job(self, wf: Path) -> dict[str, Any]:
        """Open a workflow directory on the worker (the body of ``open``, shared with ``create``)."""
        self._close_project()
        ...
        return {"workflow_dir": str(wf), "name": prj.record.data.get("name"),
                "run_state": run.state if run else "idle", "needs_recovery": self._needs_recovery}

    def open(self, workflow_dir: str | Path) -> None:
        self.submit("open", lambda: self._open_job(Path(workflow_dir)))

    def create(self, *, name: str | None = None, asset_name: str | None = None,
               project_dir: str | Path | None = None, workflow_dir: str | Path | None = None,
               first_component: str | None = None, preset_id: str | None = None, user: str = "user") -> None:
        """The CLI's ``new`` (or ``preset create``) followed by ``open``, as one job. Creating never reads a
        reference or opens a .blend (R-93); an existing directory is an error, never a silent reopen."""
        def job() -> dict[str, Any]:
            cfg = self.config
            if preset_id:
                preset = cfg.presets.get(preset_id)
                if preset is None:
                    raise ValueError(f"unknown preset {preset_id!r}; known: {', '.join(cfg.presets) or 'none'}")
                prj = Project.create_from_preset(preset_id, preset, workflow_dir=workflow_dir or None, config=cfg)
            else:
                if not name:
                    raise ValueError("a project needs a name (or a preset)")
                wf = Path(workflow_dir) if workflow_dir else default_workflow_dir(project_dir, slugify(name), cfg)
                extra = {"first_component": first_component} if first_component else {}
                prj = Project.create(wf, name=name, asset_name=asset_name or name, source_project_dir=project_dir,
                                     extra=extra, actor=f"user:{user}")
            created_dir, pid = prj.workflow_dir, prj.record.id
            prj.close()
            out = self._open_job(created_dir)
            out.update({"created": True, "project_id": pid})
            return out
        self.submit("new_project", job)

    def create_fixture(self, directory: str | Path) -> None:
        """The CLI's ``fixture create`` followed by ``open``: the disposable fixture lamp with its three planted
        defects (needs Blender). Meant for a session whose seats are scripted (``mock``), so a run spends nothing."""
        def job() -> dict[str, Any]:
            from .fixture import create_fixture

            d = Path(directory)
            scratch = Project.__new__(Project)          # the runner wants a project-like logs dir before one exists
            scratch.workflow_dir = d
            scratch.path = lambda *parts: d.joinpath(*parts)  # type: ignore[method-assign]
            runner = self._make_runner(scratch)
            if runner is None:
                raise RuntimeError("Blender was not found; the fixture project needs it (set blender.executable "
                                   "in the builder settings or install Blender under C:\\Program Files\\Blender Foundation)")
            prj, info = create_fixture(d, runner)
            pid = prj.record.id
            prj.close()
            out = self._open_job(Path(info["workflow_dir"]))
            out.update({"created": True, "fixture": True, "defects": list(info.get("defects") or []),
                        "project_id": pid})
            return out
        self.submit("new_fixture", job)

    def close_project(self) -> None:
        """Close the open project without ending the session (no snapshot follows: there is no engine)."""
        def job() -> dict[str, Any]:
            had = self.project is not None
            self._close_project()
            return {"closed": had}
        self.submit("close_project", job)
