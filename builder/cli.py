...
Agents are the real claude, gemini, and codex CLI adapters named in the builder settings file (``agents``), or
...
class Ctx:
    ...
    def runner(self, cfg: BuilderConfig, project: Project, *, required: bool = True) -> Any:
        ...
        if exe is None:
            if required:
                raise CliError("Blender was not found; set blender.executable in the builder settings file (--config, or "
                               "sessions\\builder.json) or install Blender under "
...
def cmd_preset(args: argparse.Namespace, ctx: Ctx) -> int:
    ...
    if args.preset_cmd == "list":
        if not cfg.presets:
            ctx.say("no presets under the builder settings file's presets key")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    def wf(sp: argparse.ArgumentParser) -> None:
        ...
        sp.add_argument("--config", help="builder settings file, .json or .yaml (default: $ALLOY_BUILDER_CONFIG, then ./sessions/builder.json, else built-in defaults)")
