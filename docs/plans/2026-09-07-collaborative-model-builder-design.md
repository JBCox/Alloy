...
## 12e. Phase 4 construction notes (2026-09-08, hardening, harness, GUI follow-ups, docs, packaging)

Built against Section 8.4, 8.5, 9.4, and the spec's Phase 4 list. The owner-driven live concept pass (12c, revised proposal above) was re-reported at the start of the session and is still awaiting approval; nothing was spent. Decisions made while building:

1. **Recorded PIDs and orphan reaping** (R-22, R-47). `process.run_process` gains `on_spawn(pid)`, called on the calling thread right after the Job Object assignment; `Operations.execute` records the pid on the operation while Blender runs (journal `op.spawned`) and clears it when apply returns. `reconcile_all` reaps an orphan only when the recorded pid is alive **and** its command line (CIM query, `process.command_line`) still names the operation id, because pids are reused; a live pid that belongs to another program is left alone and journaled (`op.orphan_not_killed`). The kill is `kill_tree` with confirmation (`op.orphan_killed`). On this machine a host death kills Blender through the Job Object anyway (verified on real Blender: the edit-boundary orphan was already gone); the reaper covers the case where the job assignment failed.
2. **Interrupted is not never-started.** A `running`/`validating` operation with no staged output and a recorded pid (or state) is classified `failed` with `recovery=interrupted`, not `never_started`; the staging directory is removed; nothing was promoted. `never_started` is reserved for operations that were created and never spawned.
3. **Tasks and findings return to a resumable state** (R-23, R-47). `Engine._reset_interrupted` blocks `in_progress` tasks, marks a `correcting` finding's last attempt `uncertain`, returns the finding to `open` with a `recovery_notes` entry (task, attempt, operations and their states), and releases the assembly grant when its holder's latest task is blocked and nothing is in flight (R-44). It runs from `recover()` and, in-process, at the top of `_stage_correct` and `_stage_build`, which removes an oscillation found while testing: a finding left `correcting` by a failed correction made `_stage_correct` skip it and `_stage_gate` send the run back, gate to correct, until `max_steps`.
4. **The next packet names what never applied** (R-47). Build and correction packets carry a section "Interrupted operations against the current base revision (never applied)" listing every operation attempted against the current base that ended uncertain, cancelled, or interrupted, with its quarantine directory and the sentence that the base revision is unchanged. Operations against an older base are not listed (a later commit superseded them), which the real-Blender recovery test exercises.
5. **A retried build stays with its seat.** Blocked, interrupted build tasks do not advance the builder alternation (`_assign`), so the seat whose packet names the interruption retries it.
6. **Stall detection** (R-85, R-88, stop reason `stalled`). `limits.stall_steps` (default 12; zero disables) counts consecutive loop steps whose progress key is unchanged. The key (`Engine._progress_key`) is evidence-supported progress only: valid observations, briefs, plan done, build and review tasks done, findings recorded, findings closed or waived, findings reassessed, components ready for review, acceptances. Committed revisions and renders are deliberately absent ("increasing detail is not progress"), so an unverified correction loop with unlimited attempts stops as `stalled` after `stall_steps` steps (deterministic test), while the reassessment path with `attempts_per_finding: 2` uses four steps and never trips the default. The counter and key persist in the tracker snapshot.
7. **Editable limits** (R-85, R-89). `LimitTracker.apply_limits` validates names against `DEFAULT_LIMITS`, numbers, non-negative; a rejected batch changes nothing. `Engine.apply_limits` mirrors the change on the run record and journals `run.limits_changed` (or `limits.changed` without a run). A `limits` control request is consumed at the loop's safe boundary; `Engine.consume_controls(kinds=("limits",))` applies queued edits when no loop is polling, and `Store.pop_controls(kinds=...)` filters so an idle edit never swallows a queued pause or cancel. CLI verb `limits <wf> [--set name=value ...]` applies at once to a stopped run and queues for an active one; the GUI Run tab has a Limits card whose Apply submits only the changed entries; `resume()` re-checks the limits, so "raise the limit and resume" is the documented flow (verified on real Blender with `attempts_per_finding` 2 to 3).
8. **Efficiency harness** (R-84, `builder/harness.py`, verbs `harness run <dir>` and `harness report <wf>`). Packet delivery is measured from the packet manifests (file bytes by role) plus the prompt, working agreement, and reply bytes the engine now records on every invocation (`invocation.delivered`). Transcript replay is *simulated* from the same turns: each turn re-delivers every earlier packet text and reply and every image delivered before (one shared conversation, as Alloy's chat modes concatenate contributions); nothing is sent, so its token count is unknown and never estimated. Tokens are summed only from the providers' own measured usage (unknown otherwise; scripted seats report none). The report prints absolute bytes side by side, the evidence files, coverage rows, and verified closures preserved (identical by construction), and the withheld material a shared transcript would have exposed (the reviewer's turn would carry the builder's self-assessment, R-11). No percentage and no equivalence claim; the tests assert the text contains neither. The fixture screenplay moved into `builder.harness.fixture_screenplay`; `tests/_screenplay.py` re-exports it so the tests and the harness cannot drift.
9. **Target regions by dragging** (R-26, R-27). `ImagePane` has a one-shot region mode: a drag draws a rubber band and, on release, `canvas_to_original` undoes the presentation scale and centring offset and clamps to the image, so the stored bbox is in original pixels whatever the zoom. Existing regions are drawn on the reference and listed as child rows of the reference in the Project tab; `BuilderSession.add_region` calls `References.add_region` on the worker (validation errors surface as job errors). Found while testing: Tk drops synthetic pointer events on a withdrawn window, so the drag test shows the window for the drag (an unlocked screen is required, as for screenshots).
10. **Concept tab width.** The concept image pair starts at 320x240 per pane and the column split for the Concept tab is 16/22/62 percent (`SASH_FRACTIONS`), against 28/42/30 for the other tabs.
11. **Packaging.** `alloy.spec`: `PIL` removed from the excludes; `builder` (package) and `builder/blender/scripts` added as data because the scripts run inside Blender's own Python from files, never from the PYZ; hidden imports for the lazily imported `builder.*`, `gui.builder_view`, `PIL.Image`, `PIL.ImageTk`, `sqlite3`. Launch scripts `scripts/builder.bat` (CLI) and `scripts/model-builder.bat` (window). PyInstaller is not installed and no build was attempted; `tests/test_packaging.py` checks the spec as text and runs the CLI launch script.
12. **Documentation** under `docs/builder/` (index plus thirteen pages per spec Section 8 and the concept stage), written from the code and the recorded live facts; unverified integrations are named as such.
13. **Found on real Blender, fixed with a deterministic test first.** After an `attempt_limit` stop the correction stage had been set to `done`, so "raise the limit and resume" exited at once; the stage now stays at `correct`, and the reassigned seat runs the materially different approach after the limit is raised (`tests/test_engine.py::test_raising_the_attempt_limit_and_resuming_runs_the_materially_different_approach`, then `tests/test_blender_limits.py` on the fixture).
14. **Tests.** 300 collected: 277 deterministic (all pass; 3 read the installed CLIs with `--help`/`--version`; 15 need a display through the session `tk_root` and skip with a reason otherwise), 22 real-Blender (marker `blender`: the 20 from Phase 3 plus `test_blender_recovery.py` and `test_blender_limits.py`), 1 live (marker `live`, skipped unless `ALLOY_LIVE=1`). New this phase: `test_recovery.py` (8), `test_harness.py` (6), `test_packaging.py` (4), and additions in `test_limits.py`, `test_engine.py`, `test_cli.py`, `test_viewmodel.py`, `test_builder_view.py`.
15. **Found while documenting, fixed test-first.** (a) A local-only preflight report (live tiers `not_run`) matched the cache key and would have let `start` run real adapters unverified; `Engine.start`, the CLI, and the session now refuse with the `preflight --live` remediation (R-21). (b) The Ctrl+C behaviour of Section 9.1 was never implemented; `cli.interrupt_handler` now pauses on the first and cancels on the second. (c) Open: a `new` project has no starting revision (only `fixture create` and `checkpoint restore` register one), so a real project needs a verb that registers an initial `.blend` (or an empty scene) as revision 0 after a Blender validation; for the E10 preset that means copying the owner's source into `revisions/` during an authorized run. Proposed for the owner's decision, not built.
16. **Not built.** The claude activity signal (12c item 14c) still needs a paid probe; the live concept pass awaits approval; the Alloy settings editor's Builder tab (not in this phase's scope) does not list `stall_steps`, but its deep-merge save preserves the key from `config.yaml`.

## 12f. First enemy run: prerequisites and the E11 Mast project (2026-09-08)

Built test-first against the kickoff "build the next enemy end to end". Decisions made while building:

1. **Revision 0 verbs** (`builder/source.py`, CLI `source register <wf> <file.blend>` and `source empty <wf>`; closes 12e item 15c). `register_source` runs two separate Blender processes on the owner's file (`identities.py`, then `validate.py` with `allow_unmapped`), copies it byte for byte through `Operations.register_revision` (new `extra` kwarg carries a `source` block: path, size, mtime, sha256, `unmapped_geometry`, object count, reopen result), journals `source.validated` and `source.registered` (or `source.rejected`), and creates `part` records for tagged objects and instances. The file is only ever read: bytes and mtime unchanged, no `.blend1` beside it (asserted on real Blender). Untagged geometry is recorded, never tagged in place: tagging would modify the owner's file and a revision must be the file the owner gave. `register_empty` builds one tagged collection (`col_model`) and no objects through `build_scene.py` under `staging/`, validates, and moves it into `revisions/`. Both refuse once a revision exists (R-40). The window has both on the Project tab; `status` prints `revision: rev_...`. Tests: `tests/test_source.py` (9, fake runner), `tests/test_blender_source.py` (3, real Blender: owner-like file with an untagged object, corrupted file refused with Blender's error, empty scene measures empty).
2. **Assignment overrides** (`start --assign role=SEAT`, `builder.assignments`, the Run tab entry). `Engine.resolve_assignments` merges the config key (source `config`) with explicit overrides (source `cli`, which win) and validates strictly: an unknown role or seat on `--assign` is an `EngineFailure` before the run record is created; in the config key it is a warning. The run record carries `assignment_overrides` and `assignment_override_sources`; every task decided by an override records the rationale `user override (<source>: <role>=<seat>) (R-8); R-107 still applies: <seat> never reviews, verifies, or reassesses its own operation`; `status`, `status --json`, the viewmodel `stage.assignment_overrides`, and the Stage card show one line per override. `roles.assign` is unchanged: `--assign build=B --assign reviewer=B` builds with B and stops the run at the review stage with `execution_failure` and the R-107 reason (`_role_of` appends "assignment override reviewer=B cannot be honoured for this task"). Assignable roles: `brief`, `plan`, `build`, `corrector`, `reviewer`, `verifier`, `reassessor` (`config.ASSIGNABLE_ROLES`); the art director is not overridable here (R-108 rotation). Tests: `tests/test_assignments.py` (6), three window tests in `tests/test_builder_view.py`, the settings round trip now covers `assignments`.
3. **The image seat in practice.** The kickoff's preferred path (Claude driving the owner's ChatGPT tab one prompt at a time, a download confirmation per image) produced one anchor candidate in about ten minutes and was stopped by the owner: the concept stage's sequence (4 anchor candidates, pick, canon description, one view per prompt, studies only after the plan names parts) is far too slow and interaction-heavy when the owner can produce whole reference sheets directly, and close-up references cannot be requested before the construction plan exists (a gap: studies need a part id). The owner generated four composite sheets (master turnaround with head/torso/leg close-ups; head and upper systems with head orthographics and modular components; torso, legs, joints and hydraulics; foot, lower terminals and parts library) in ChatGPT and handed over the files. They entered as owner targets (R-99, A.1: canon by the owner's decision, `evidence_of_original` yes by kind, with the provenance written in each reference's notes), `composite` with target regions and detail crops in original pixels (R-27). The text-start attempt is kept aside (`e11-mast-head.attempt1-text-start`, about 3.6 USD in claude estimates) because a concept plan cannot be removed and `_stage_intake` refuses while one is not complete; the project was recreated and the live preflight re-run (about 2.6 USD, codex unknown). Recorded for the design: the codex CLI's `--image` is input only (no image generation), and an owner-driven sheet workflow needs (a) region crops delivered as evidence, which today are recorded but not cut into packets, and (b) studies requestable before the plan.
4. **Two live findings from the first build calls (each changed code or config).** (a) The 900 s response timeout is too short for a codex build at `xhigh`: the first call was killed at 902 s while composing its operation (the CLI had also spent about twenty shell commands reading Alloy's own source to learn the validation rules the packet states, and OpenAI's stream dropped four times). The owner raised `provider_timeouts` to 3600 s and then 7200 s for this run (the values are read at engine start; a change applies from the next `resume`). (b) The retry completed in 18 minutes with a full reply (one operation, 26 target parts, a 15.6 KB script, usage reported) and the codex adapter discarded it as `provider_error` because the JSONL carried `error` events ("Reconnecting... n/5 ... websocket closed by server") before `turn.completed`. Fixed test-first (`tests/test_adapters.py::test_codex_stream_reconnect_notices_before_a_completed_turn_are_warnings_not_a_failure`, fake CLI mode `reconnect_then_ok`; `error_event_only` proves an `error` with no completed turn is still a failure): with a completed turn the notices are warnings, never a reason to drop the reply. Cost of the two lost calls: codex unknown; about 33 minutes of wall clock.
5. **The run.** `start --unattended --assign build=B` on the recreated project: RUN-RESULTS-PLACEHOLDER
