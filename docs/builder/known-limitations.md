...
- The app has no packaged build; it runs from the checkout (`python app.py`). The builder's Blender scripts run from files inside Blender's own Python, so any future packaging must ship `builder/blender/scripts/*.py` as loose files.
- `builder.cmd` at the repo root wraps `python -m builder` and points `ALLOY_BUILDER_CONFIG` at `sessions\builder.json`. The Tk window, its launch script and `alloy.spec` stayed on the `legacy-tk` branch.
