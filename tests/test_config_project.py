...
# --- D9: the builder key survives Alloy's own config loader and the settings editor save path ----------

BUILDER_YAML = """default_ai: claude
ais:
  claude:
...
def test_config_load_keeps_builder_key_and_existing_behaviour(tmp_path):
    from config import Config

    p = tmp_path / "config.yaml"
    p.write_text(BUILDER_YAML, encoding="utf-8")
    cfg = Config.load(p)
    assert cfg.default_ai == "claude" and cfg.timeout == 300 and "claude" in cfg.ais
    assert cfg.builder["presets"]["demo"]["name"] == "Demo"
    out = tmp_path / "saved.yaml"
    cfg.save(out)
    saved = yaml.safe_load(out.read_text(encoding="utf-8"))
    assert saved["builder"]["agents"]["A"]["model"] == "fable"
    assert saved["ais"]["claude"]["command"] == "claude -p {message}"


def test_builder_key_survives_settings_editor_merge_paths(tmp_path):
    try:
        from gui.settings import REPLACE_SECTIONS, deep_merge
    except ImportError as exc:   # the settings-editor merge helpers belong to the in-progress GUI work
        pytest.skip(f"settings editor merge helpers not present in this tree: {exc}")

    p = tmp_path / "config.yaml"
    p.write_text(BUILDER_YAML, encoding="utf-8")
    collected = {"default_ai": "gemini", "ais": {"claude": {"command": "claude -p {message}", "color": "cyan",
                                                            "enabled": False}},
                 "custom_templates": {}, "execution": {"timeout": 120}}
    # plain-merge path (_merged_config)
    base = yaml.safe_load(p.read_text(encoding="utf-8"))
    merged = deep_merge(base, collected, REPLACE_SECTIONS)
    assert merged["builder"]["presets"]["demo"]["name"] == "Demo" and merged["default_ai"] == "gemini"
    # ruamel round-trip path (_save_preserving_comments)
    ...
    with open(p, "r", encoding="utf-8") as f:
        doc = ry.load(f)
    deep_merge(doc, collected, REPLACE_SECTIONS)
    buf = io.StringIO()
    ry.dump(doc, buf)
    text = buf.getvalue()
    reloaded = yaml.safe_load(text)
    assert reloaded["builder"]["agents"]["A"]["reasoning"] == "max"
    assert reloaded["execution"]["timeout"] == 120
...
def test_importing_builder_writes_nothing(tmp_path, monkeypatch):
    ...
    before = sorted(str(p) for p in Path(__file__).resolve().parent.parent.glob("*"))
    ...
    importlib.reload(bc)
    importlib.reload(bp)
    after = sorted(str(p) for p in Path(__file__).resolve().parent.parent.glob("*"))
    assert before == after
