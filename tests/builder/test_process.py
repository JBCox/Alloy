...
def test_child_never_inherits_claude_or_anthropic_vars(workdir, monkeypatch):
    """relay.clean_env's rule: a claude CLI that inherits the host session's CLAUDE*/ANTHROPIC* variables
    (Alloy itself launched from inside a Claude Code session) reports "Not logged in". The child must never
    see them, while an explicit env_extra still passes through."""
    monkeypatch.setenv("CLAUDECODE", "1")
    monkeypatch.setenv("ANTHROPIC_API_KEY", "x")
    monkeypatch.setenv("claude_config_dir", "y")      # case-insensitive, like relay
    res = _run(workdir, "echo", env_extra={"ALLOY_TEST_MARKER": "kept"})
    assert res.outcome == "ok"
    payload = json.loads(Path(res.stdout_path).read_text(encoding="utf-8").splitlines()[0])
    assert payload["host_env"] == []
    assert payload["env_marker"] == "kept"
